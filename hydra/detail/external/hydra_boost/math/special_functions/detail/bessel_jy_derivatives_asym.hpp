...
// functions, as x -> INF.
#ifndef HYDRA_BOOST_MATH_SF_DETAIL_BESSEL_JY_DERIVATIVES_ASYM_HPP
#define HYDRA_BOOST_MATH_SF_DETAIL_BESSEL_JY_DERIVATIVES_ASYM_HPP
...
inline T asymptotic_bessel_derivative_amplitude(T v, T x)
{
   ...
   return sqrt(s * 2 / (hydra_boost::math::constants::pi<T>() * x));
}
...
inline bool asymptotic_bessel_derivative_large_x_limit(const T& v, const T& x)
{
   ...
   //
   return (std::max)(T(fabs(v)), T(1)) < x * sqrt(hydra_boost::math::tools::forth_root_epsilon<T>());
}

}}} // namespaces

#endif // HYDRA_BOOST_MATH_SF_DETAIL_BESSEL_JY_DERIVATIVES_ASYM_HPP
