...
struct range
{
      ...
      result(r.result), comp(r.comp), grain_size(r.grain_size)
  {
    ...
    size_t n1 = hydra_thrust::distance(first1, last1);
    size_t n2 = hydra_thrust::distance(first2, last2);
    ...
    {
      mid1 += n1 / 2;
      mid2 = hydra_thrust::lower_bound(hydra_thrust::seq, first2, last2, raw_reference_cast(*mid1), comp);
    }
    else
    {
      mid2 += n2 / 2;
      mid1 = hydra_thrust::upper_bound(hydra_thrust::seq, first1, last1, raw_reference_cast(*mid2), comp);
    }
    ...
    first1 = mid1;
    first2 = mid2;
    result += hydra_thrust::distance(r.first1, mid1) + hydra_thrust::distance(r.first2, mid2);
  }
  ...
  {
    return static_cast<size_t>(hydra_thrust::distance(first1, last1) + hydra_thrust::distance(first2, last2)) > grain_size;
  }
};
...
struct body
{
  ...
  {
    hydra_thrust::merge(hydra_thrust::seq,
                  r.first1, r.last1,
                  r.first2, r.last2,
                  r.result,
                  r.comp);
...
struct range
{
  ...
  {
    // we can assume n1 and n2 are not both 0
    size_t n1 = hydra_thrust::distance(keys_first1, keys_last1);
    size_t n2 = hydra_thrust::distance(keys_first2, keys_last2);
    ...
    {
      mid1 += n1 / 2;
      mid2 = hydra_thrust::lower_bound(hydra_thrust::seq, keys_first2, keys_last2, raw_reference_cast(*mid1), comp);
    }
    else
    {
      mid2 += n2 / 2;
      mid1 = hydra_thrust::upper_bound(hydra_thrust::seq, keys_first1, keys_last1, raw_reference_cast(*mid2), comp);
    }
    ...
    keys_first1 = mid1;
    keys_first2 = mid2;
    values_first1 += hydra_thrust::distance(r.keys_first1, mid1);
    values_first2 += hydra_thrust::distance(r.keys_first2, mid2);
    keys_result += hydra_thrust::distance(r.keys_first1, mid1) + hydra_thrust::distance(r.keys_first2, mid2);
    values_result += hydra_thrust::distance(r.keys_first1, mid1) + hydra_thrust::distance(r.keys_first2, mid2);
  }
  ...
  bool is_divisible(void) const
  {
    return static_cast<size_t>(hydra_thrust::distance(keys_first1, keys_last1) + hydra_thrust::distance(keys_first2, keys_last2)) > grain_size;
  }
};
                     ...
                     StrictWeakOrdering comp)
{
  ...
  hydra_thrust::advance(result, hydra_thrust::distance(first1, last1) + hydra_thrust::distance(first2, last2));
               ...
               StrictWeakOrdering comp)
{
  ...
  hydra_thrust::advance(keys_result,   hydra_thrust::distance(keys_first1, keys_last1) + hydra_thrust::distance(keys_first2, keys_last2));
  hydra_thrust::advance(values_result, hydra_thrust::distance(keys_first1, keys_last1) + hydra_thrust::distance(keys_first2, keys_last2));

  return hydra_thrust::make_pair(keys_result,values_result);
}
