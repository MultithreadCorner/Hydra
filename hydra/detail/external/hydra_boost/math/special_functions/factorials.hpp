...
T rising_factorial_imp(T x, int n, const Policy& pol)
{
   ...
   {
      if(n < 0)
         return static_cast<T>(-hydra_boost::math::tgamma_delta_ratio(x + 1, static_cast<T>(-n), pol));
      else
         return 0;
   }
   if((x < 1) && (x + n < 0))
   {
      const auto val = static_cast<T>(hydra_boost::math::tgamma_delta_ratio(1 - x, static_cast<T>(-n), pol));
   ...
   // use case:
   //
   return 1 / static_cast<T>(hydra_boost::math::tgamma_delta_ratio(x, static_cast<T>(n), pol));
}
...
inline T falling_factorial_imp(T x, unsigned n, const Policy& pol)
{
   ...
   if(x <= n - 1)
   {
      ...
      if(n2 == xp1)
         return 0;
      auto result = static_cast<T>(hydra_boost::math::tgamma_delta_ratio(xp1, -static_cast<T>(n2), pol));
      x -= n2;
      result *= x;
      ++n2;
      if(n2 < n)
