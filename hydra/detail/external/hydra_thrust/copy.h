 ...
 *  to copy from one range to another using the \p hydra_thrust::device parallelization policy:
 *
 *  \code
 ...
 *  ...
 *
 *  hydra_thrust::device_vector<int> vec0(100);
 *  hydra_thrust::device_vector<int> vec1(100);
 *  ...
 *
 *  hydra_thrust::copy(hydra_thrust::device, vec0.begin(), vec0.end(), vec1.begin());
 *
 ...
 *  ...
 *  size_t n = 100;
 *  hydra_thrust::device_vector<int> vec0(n);
 *  hydra_thrust::device_vector<int> vec1(n);
 *  ...
 *  hydra_thrust::copy_n(hydra_thrust::device, vec0.begin(), n, vec1.begin());
 *
 *  // vec1 is now a copy of vec0
 *  \endcode
 *
 *  \see http://www.sgi.com/tech/stl/copy_n.html
 *  \see hydra_thrust::copy
 */
 ...
 *  ...
 *
 *  hydra_thrust::copy(vec0.begin(), vec0.end(),
 *               vec1.begin());
 *
 *  // vec1 is now a copy of vec0
 *  \endcode
 ...
 *  hydra_thrust::device_vector<int> vec1(n);
 *  ...
 *  hydra_thrust::copy_n(vec0.begin(), n, vec1.begin());
 *
 *  // vec1 is now a copy of vec0
 *  \endcode
 *
 ...
 *  to copy even numbers to an output range using the \p hydra_thrust::host parallelization policy:
 *
 *  \code
 *  #include <hydra/detail/external/hydra_thrust/copy.h>
 *  #include <hydra/detail/external/hydra_thrust/execution_policy.h>
 ...
 *  int result[4];
 *
 *  hydra_thrust::copy_if(hydra_thrust::host, V, V + N, result, is_even());
 *
...
__host__ __device__
  OutputIterator copy_if(const hydra_thrust::detail::execution_policy_base<DerivedPolicy> &exec,
                         InputIterator first,
                         InputIterator last,
                         OutputIterator result,
                         Predicate pred);
 ...
 *  int result[4];
 *
 *  hydra_thrust::copy_if(V, V + N, result, is_even());
 *
 *  // V remains {-2, 0, -1, 0, 1, 2}
 *  // result is now {-2, 0, 0, 2}
 *  \endcode
 ...
 *
 *  The following code snippet demonstrates how to use \p copy_if to perform stream compaction
 *  to copy numbers to an output range when corresponding stencil elements are even using the \p hydra_thrust::host execution policy:
 *
 *  \code
 *  #include <hydra/detail/external/hydra_thrust/copy.h>
 *  #include <hydra/detail/external/hydra_thrust/execution_policy.h>
 ...
 *  int result[4];
 *
 *  hydra_thrust::copy_if(hydra_thrust::host, data, data + N, stencil, result, is_even());
 *
 ...
 *  int result[4];
 *
 *  hydra_thrust::copy_if(data, data + N, stencil, result, is_even());
 *
 *  // data remains    = { 0, 1,  2, 3, 4, 5};
 *  // stencil remains = {-2, 0, -1, 0, 1, 2};
 *  // result is now     { 0, 1,  3, 5}
