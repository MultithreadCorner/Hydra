...
T binomial_ccdf(T n, T k, T x, T y, const Policy& pol)
{
   ...
   else
   {
      ...
      if(start <= k + 1)
         start = itrunc(k + 2);
      result = static_cast<T>(pow(x, T(start)) * pow(y, n - T(start)) * hydra_boost::math::binomial_coefficient<T>(itrunc(n), itrunc(start), pol));
      if(result == 0)
      {
         ...
         {
            result += static_cast<T>(pow(x, static_cast<T>(i)) * pow(y, n - i) * hydra_boost::math::binomial_coefficient<T>(itrunc(n), itrunc(i), pol));
         }
      }
      else
      {
...
T ibeta_derivative_imp(T a, T b, T x, const Policy& pol)
{
   ...
   T y = (1 - x) * x;
   T f1;
   if (!(hydra_boost::math::isinf)(1 / y))
   {
      f1 = ibeta_power_terms<T>(a, b, x, 1 - x, lanczos_type(), true, pol, 1 / y, function);
   }
   else
   {
      return (a > 1) ? 0 : (a == 1) ? 1 / hydra_boost::math::beta(a, b, pol) : policies::raise_overflow_error<T>(function, nullptr, pol);
   }
   ...
   beta(RT1 a, RT2 b, A arg)
{
   ...
   return static_cast<ReturnType>(hydra_boost::math::detail::beta(a, b, arg, static_cast<tag*>(nullptr)));
}
