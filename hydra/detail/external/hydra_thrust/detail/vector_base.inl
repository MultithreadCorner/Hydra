...
{
  size_type new_size = hydra_thrust::distance(first, last);
         ...
         m_size(0)
{
  ...
  typedef hydra_thrust::detail::is_integral<InputIterator> Integer;
    ...
    ::resize(size_type new_size)
{
  if(new_size < size())
  {
    ...
    hydra_thrust::advance(new_end, new_size);
    erase(new_end, end());
  } // end if
  else
  {
      ...
      ::end(void)
{
  ...
  hydra_thrust::advance(result, size());
  return result;
} // end vector_base::end()
    ...
    ::erase(iterator first, iterator last)
{
  ...
  iterator i = hydra_thrust::detail::overlapped_copy(last, end(), first);
...
template<typename T, typename Alloc>
  void vector_base<T,Alloc>
    ::swap(vector_base &v)
{
  hydra_thrust::swap(m_storage,  v.m_storage);
  hydra_thrust::swap(m_size,     v.m_size);
      ...
      ::assign(InputIterator first, InputIterator last)
{
  ...
  typedef typename hydra_thrust::detail::is_integral<InputIterator> integral;
      ...
      ::insert(iterator position, const T &x)
{
  ...
  size_type index = hydra_thrust::distance(begin(), position);
  ...
  iterator result = begin();
  hydra_thrust::advance(result, index);
  return result;
                    ...
                    ForwardIterator last)
{
  if(first != last)
  {
    ...
    const size_type num_new_elements = hydra_thrust::distance(first, last);
    if(capacity() - size() >= num_new_elements)
    {
      ...
      if(num_displaced_elements > num_new_elements)
      {
        ...
        hydra_thrust::detail::overlapped_copy(position, old_end - num_new_elements, old_end - copy_length);

        // finally, copy the range to the insertion point
        hydra_thrust::copy(first, last, position);
      } // end if
      else
      {
        ForwardIterator mid = first;
        hydra_thrust::advance(mid, num_displaced_elements);
        ...
        hydra_thrust::copy(first, mid, position);
      } // end else
    } // end if
    else
    {
      ...
      size_type new_capacity = old_size + hydra_thrust::max HYDRA_THRUST_PREVENT_MACRO_SUBSTITUTION (old_size, num_new_elements);

      // allocate exponentially larger new storage
      new_capacity = hydra_thrust::max HYDRA_THRUST_PREVENT_MACRO_SUBSTITUTION <size_type>(new_capacity, 2 * capacity());
    ...
    ::append(size_type n)
{
  if(n != 0)
  {
    ...
    else
    {
      ...
      // compute the new capacity after the allocation
      size_type new_capacity = old_size + hydra_thrust::max HYDRA_THRUST_PREVENT_MACRO_SUBSTITUTION (old_size, n);
  ...
  if(n != 0)
  {
    if(capacity() - size() >= n)
    {
      ...
      if(num_displaced_elements > n)
      {
        ...
        hydra_thrust::detail::overlapped_copy(position, old_end - n, old_end - copy_length);

        // finally, fill the range to the insertion point
        hydra_thrust::fill_n(position, n, x);
      } // end if
      else
      {
        ...
        hydra_thrust::fill(position, old_end, x);
      } // end else
    } // end if
    else
    {
  ...
  template<typename RandomAccessIterator>
    void vector_base<T,Alloc>
      ::range_assign(RandomAccessIterator first,
                     ...
                     hydra_thrust::random_access_traversal_tag)
{
  const size_type n = hydra_thrust::distance(first, last);
  ...
  else if(size() >= n)
  {
    ...
    iterator new_end = hydra_thrust::copy(first, last, begin());
  ...
  else
  {
    ...
    hydra_thrust::advance(mid, size());
    hydra_thrust::copy(first, mid, begin());
    ...
    ::fill_assign(size_type n, const T &x)
{
  ...
  {
    // fill to existing elements
    iterator new_end = hydra_thrust::fill_n(begin(), n, x);
                          ...
                          storage_type &new_storage)
{
  ...
  // allocate exponentially larger new storage
  size_type allocated_size = hydra_thrust::max<size_type>(requested_size, 2 * capacity());

  // do not exceed maximum storage
  allocated_size = hydra_thrust::min<size_type>(allocated_size, max_size());
  ...
  catch(...)
  {
    ...
    hydra_thrust::advance(new_storage_end, requested_size);
...
{
  return hydra_thrust::equal(first1, last1, first2);
}
...
{
  typename hydra_thrust::iterator_difference<InputIterator1>::type n = hydra_thrust::distance(first1,last1);

  typedef typename hydra_thrust::iterator_system<InputIterator1>::type FromSystem1;
  typedef typename hydra_thrust::iterator_system<InputIterator2>::type FromSystem2;
  ...
  hydra_thrust::host_system_tag to_system;
  hydra_thrust::detail::move_to_system<InputIterator1, FromSystem1, hydra_thrust::host_system_tag> rng1(from_system1, to_system, first1, last1);
  hydra_thrust::detail::move_to_system<InputIterator2, FromSystem2, hydra_thrust::host_system_tag> rng2(from_system2, to_system, first2, first2 + n);

  return hydra_thrust::equal(rng1.begin(), rng1.end(), rng2.begin());
}
                  ...
                  InputIterator2 first2)
{
  ...
  return vector_equal(first1, last1, first2,
    hydra_thrust::detail::is_same<system1,system2>());
}
