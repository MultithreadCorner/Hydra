  ...
  struct body
{
  ...
  void operator()(const ::tbb::blocked_range<Size> &r) const
  {
    ...
    hydra_thrust::for_each_n(hydra_thrust::system::detail::sequential::seq, m_first + r.begin(), r.size(), m_f);
  } // end operator()()
}; // end body
...
template<typename DerivedPolicy,
  ...
  RandomAccessIterator for_each(execution_policy<DerivedPolicy> &s,
                                ...
                                UnaryFunction f)
{
  return tbb::detail::for_each_n(s, first, hydra_thrust::distance(first,last), f);
} // end for_each()
