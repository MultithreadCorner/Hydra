 ...
 *  Instead of the backend system specified by \p HYDRA_THRUST_DEVICE_SYSTEM, \p pointer's
 ...
 */
template<typename Element, typename Tag, typename Reference = hydra_thrust::use_default, typename Derived = hydra_thrust::use_default>
  class pointer
{
  public:
 ...
 *          The pointer must be deallocated with \p hydra_thrust::free.
 *
 *  \tparam DerivedPolicy The name of the derived execution policy.
 *
 *  \pre \p DerivedPolicy must be publically derived from <tt>hydra_thrust::execution_policy<DerivedPolicy></tt>.
 *
 ...
 *  ...
 *  // allocate some memory with hydra_thrust::malloc
 *  const int N = 100;
 *  hydra_thrust::device_system_tag device_sys;
 *  hydra_thrust::pointer<void,hydra_thrust::device_space_tag> void_ptr = hydra_thrust::malloc(device_sys, N);
 *
 *  // manipulate memory
 *  ...
 *
 *  // deallocate void_ptr with hydra_thrust::free
 *  hydra_thrust::free(device_sys, void_ptr);
 *  \endcode
 *
 *  \see free
 *  \see device_malloc
...
__host__ __device__
pointer<void,DerivedPolicy> malloc(const hydra_thrust::detail::execution_policy_base<DerivedPolicy> &system, std::size_t n);
 ...
 *  #include <hydra/detail/external/hydra_thrust/memory.h>
 *  ...
 *  // allocate storage for 100 ints with hydra_thrust::malloc
 *  const int N = 100;
 *  hydra_thrust::device_system_tag device_sys;
 *  hydra_thrust::pointer<int,hydra_thrust::device_system_tag> ptr = hydra_thrust::malloc<int>(device_sys, N);
 *
 *  // manipulate memory
 *  ...
 *
 *  // deallocate ptr with hydra_thrust::free
 *  hydra_thrust::free(device_sys, ptr);
 *  \endcode
 *
 *  \see free
 *  \see device_malloc
...
__host__ __device__
pointer<T,DerivedPolicy> malloc(const hydra_thrust::detail::execution_policy_base<DerivedPolicy> &system, std::size_t n);
 ...
 *  #include <hydra/detail/external/hydra_thrust/memory.h>
 *  ...
 *  // allocate storage for 100 ints with hydra_thrust::get_temporary_buffer
 *  const int N = 100;
 *
 *  typedef hydra_thrust::pair<
 *    hydra_thrust::pointer<int,hydra_thrust::device_system_tag>,
 *    std::ptrdiff_t
 *  > ptr_and_size_t;
 *
 *  hydra_thrust::device_system_tag device_sys;
 *  ptr_and_size_t ptr_and_size = hydra_thrust::get_temporary_buffer<int>(device_sys, N);
 *
 ...
 *  hydra_thrust::return_temporary_buffer(device_sys, ptr_and_size.first);
 *  \endcode
 *
 *  \see malloc
...
get_temporary_buffer(const hydra_thrust::detail::execution_policy_base<DerivedPolicy> &system, typename hydra_thrust::pointer<T,DerivedPolicy>::difference_type n);

/*! \p free deallocates the storage previously allocated by \p hydra_thrust::malloc.
 *
 *  \param system The Thrust system with which the storage is associated.
 *  \param ptr A pointer previously returned by \p hydra_thrust::malloc. If \p ptr is null, \p free
 *         does nothing.
 *
 *  \tparam DerivedPolicy The name of the derived execution policy.
 *
 *  \pre \p ptr shall have been returned by a previous call to <tt>hydra_thrust::malloc(system, n)</tt> or <tt>hydra_thrust::malloc<T>(system, n)</tt> for some type \c T.
 *
 *  The following code snippet demonstrates how to use \p free to deallocate a range of memory
 *  previously allocated with \p hydra_thrust::malloc.
 *
 *  \code
 *  #include <hydra/detail/external/hydra_thrust/memory.h>
 *  ...
...
__host__ __device__
void free(const hydra_thrust::detail::execution_policy_base<DerivedPolicy> &system, Pointer ptr);
 ...
 *
 *  \param system The Thrust system with which the storage is associated.
 *  \param p A pointer previously returned by \p hydra_thrust::get_temporary_buffer. If \p ptr is null, \p return_temporary_buffer does nothing.
 *
 *  \tparam DerivedPolicy The name of the derived execution policy.
 *
 *  \pre \p p shall have been previously allocated by \p hydra_thrust::get_temporary_buffer.
 *
...
template<typename DerivedPolicy, typename Pointer>
__host__ __device__
void return_temporary_buffer(const hydra_thrust::detail::execution_policy_base<DerivedPolicy> &system, Pointer p, std::ptrdiff_t n);
 ...
 *  \return <tt>*hydra_thrust::raw_pointer_cast(&ref)</tt>.
