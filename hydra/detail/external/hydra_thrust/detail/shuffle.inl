...
#include <hydra/detail/external/hydra_thrust/shuffle.h>
...
__host__ __device__ void shuffle(
  ...
  using hydra_thrust::system::detail::generic::shuffle;
  return shuffle(
...
__host__ __device__ void shuffle(RandomIterator first, RandomIterator last,
  ...
  typedef typename hydra_thrust::iterator_system<RandomIterator>::type System;
  System system;

  return hydra_thrust::shuffle(select_system(system), first, last, g);
}
...
__host__ __device__ void shuffle_copy(
    ...
    URBG&& g) {
  using hydra_thrust::system::detail::generic::shuffle_copy;
  return shuffle_copy(
      hydra_thrust::detail::derived_cast(hydra_thrust::detail::strip_const(exec)),
      first, last, result, g);
}
...
__host__ __device__ void shuffle_copy(RandomIterator first, RandomIterator last,
  ...
  typedef typename hydra_thrust::iterator_system<RandomIterator>::type System1;
  typedef typename hydra_thrust::iterator_system<OutputIterator>::type System2;

  System1 system1;
  System2 system2;

  return hydra_thrust::shuffle_copy(select_system(system1, system2), first, last,
                              result, g);
}
