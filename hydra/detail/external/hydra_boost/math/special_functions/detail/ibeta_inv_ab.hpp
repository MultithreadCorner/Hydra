...
#ifndef HYDRA_BOOST_MATH_SP_DETAIL_BETA_INV_AB
#define HYDRA_BOOST_MATH_SP_DETAIL_BETA_INV_AB
...
struct beta_inv_ab_t
{
   ...
   {
      return invert ? 
         p - hydra_boost::math::ibetac(swap_ab ? b : a, swap_ab ? a : b, z, Policy()) 
         : hydra_boost::math::ibeta(swap_ab ? b : a, swap_ab ? a : b, z, Policy()) - p;
   }
private:
   T b, z, p;
   bool invert, swap_ab;
...
T ibeta_inv_ab_imp(const T& b, const T& z, const T& p, const T& q, bool swap_ab, const Policy& pol)
{
   ...
   // Special cases first:
   //
   HYDRA_BOOST_MATH_INSTRUMENT_CODE("b = " << b << " z = " << z << " p = " << p << " q = " << " swap = " << swap_ab);
   if(p == 0)
   {
   ...
   if(max_iter >= policies::get_max_root_iterations<Policy>())
      return policies::raise_evaluation_error<T>("hydra_boost::math::ibeta_invab_imp<%1%>(%1%,%1%,%1%)", "Unable to locate the root within a reasonable number of iterations, closest approximation so far was %1%", r.first, pol);
      ...
      ibeta_inva(RT1 b, RT2 x, RT3 p, const Policy& pol)
{
   ...
   static const char* function = "hydra_boost::math::ibeta_inva<%1%>(%1%,%1%,%1%)";
   if(p == 0)
   {
      ...
      ibetac_inva(RT1 b, RT2 x, RT3 q, const Policy& pol)
{
   ...
   static const char* function = "hydra_boost::math::ibetac_inva<%1%>(%1%,%1%,%1%)";
   if(q == 1)
   {
      return policies::raise_overflow_error<result_type>(function, 0, Policy());
   }
      ...
      ibeta_invb(RT1 a, RT2 x, RT3 p, const Policy& pol)
{
   ...
   static const char* function = "hydra_boost::math::ibeta_invb<%1%>(%1%,%1%,%1%)";
   if(p == 0)
   {
      return tools::min_value<result_type>();
   }
...
{
   return hydra_boost::math::ibeta_inva(b, x, p, policies::policy<>());
}
...
{
   return hydra_boost::math::ibetac_inva(b, x, q, policies::policy<>());
}
...
{
   return hydra_boost::math::ibeta_invb(a, x, p, policies::policy<>());
}
...
{
   return hydra_boost::math::ibetac_invb(a, x, q, policies::policy<>());
}

} // namespace math
} // namespace hydra_boost

#endif // HYDRA_BOOST_MATH_SP_DETAIL_BETA_INV_AB
