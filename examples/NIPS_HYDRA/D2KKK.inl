...
template<unsigned int CHANNEL> class Flatte: public hydra::BaseFunctor<Flatte<CHANNEL>, hydra::complex<double>, 5>
{
    ...
    using hydra::BaseFunctor<Flatte<CHANNEL>, hydra::complex<double>, 5>::_par;
...
public:
    ...
    Flatte(hydra::Parameter const& c_re, hydra::Parameter const& c_im,hydra::Parameter const& mean, hydra::Parameter const& rho1 , hydra::Parameter const& rho2,
           double mother_mass,	double daugther1_mass, double daugther2_mass, double daugther3_mass,double radi):
            hydra::BaseFunctor<Flatte<CHANNEL>, hydra::complex<double>, 5>{c_re, c_im, mean, rho1, rho2},
            fLineShape(mean,rho1,rho2,mother_mass,daugther1_mass,daugther2_mass,daugther3_mass,radi)
    {}

    __host__ __device__ Flatte( Flatte< CHANNEL> const& other):
            hydra::BaseFunctor<Flatte<CHANNEL>, hydra::complex<double>, 5>(other),
            fLineShape(other.GetLineShape())
    {}

    __host__ __device__ inline
    Flatte< CHANNEL>&
    operator=( Flatte<CHANNEL> const& other)
    {
        if(this==&other) return *this;

        hydra::BaseFunctor<Flatte<CHANNEL>, hydra::complex<double>, 5>::operator=(other);
        fLineShape=other.GetLineShape();
    ...
    __host__ __device__ inline
    hydra::FlatteLineShape<CHANNEL> const& GetLineShape() const {	return fLineShape; }
    ...
    __host__ __device__ hydra::complex<double> Evaluate(unsigned int n, hydra::Vector4R* p)  const {
        ...
        auto r = hydra::complex<double>(_par[0],_par[1])*fLineShape((p1+p2).mass());
...
int main(int argv, char** argc)
{
    ...
    double f0X_MASS		  = 1.41478;
    double f0X_Width	  = 0.309491;
    double f0X_RC		  = 11.918 * cos(20.248 * (M_PI / 180));
    double f0X_IMC        = 11.918 * sin(20.248 * (M_PI / 180));
    ...
    //======================================================
	//Phi
	auto mass    = hydra::Parameter::Create().Name("MASS_Phi" ).Value(Phi_MASS ).Error(0.01).Limits(0.7,  1.8 ).Fixed();
	auto width   = hydra::Parameter::Create().Name("WIDTH_Phi").Value(Phi_Width).Error(0.001).Limits(1.e-5, 1.e-1).Fixed();
	...
	Resonance<1, hydra::PWave> Phi_Resonance_12(coef_re, coef_im, mass, width,
		    	D_MASS,	Kminus_MASS, Kplus_MASS, Kplus_MASS , 5.0);

	Resonance<3, hydra::PWave> Phi_Resonance_13(coef_re, coef_im, mass, width,
			    	D_MASS,	Kminus_MASS, Kminus_MASS, Kplus_MASS , 5.0);
    ...
    Resonance<3, hydra::SWave> f0X_Resonance_13(coef_ref0x, coef_imf0x, massf0x, widthf0x,
                    D_MASS,	Kminus_MASS, Kplus_MASS, Kplus_MASS , 5.0);

    auto f0X_Resonance = (f0X_Resonance_12 + f0X_Resonance_13);
    ...
    Flatte<1> f0_Resonance_12(f0_amp_real,f0_amp_imag,f0Mass,f0g1,rg1og2,D_MASS,Kminus_MASS,Kplus_MASS,Kplus_MASS,1.5);
    Flatte<3> f0_Resonance_13(f0_amp_real,f0_amp_imag,f0Mass,f0g1,rg1og2,D_MASS,Kminus_MASS,Kplus_MASS,Kplus_MASS,1.5);
	...
	//NR
	coef_re = hydra::Parameter::Create().Name("A_RE_NR" ).Value(NR_RC).Error(0.001).Limits(-100,+100);
	coef_im = hydra::Parameter::Create().Name("A_IM_NR" ).Value(NR_IMC).Error(0.001).Limits(-100,+100);
	...
	// fit
	{
        ...
        auto NR     = fcn.GetPDF().GetFunctor().GetFunctor(_4);
		...
		NR_HIST      =	histogram_component(NR, {D_MASS, Kminus_MASS, Kplus_MASS}, "NR_HIST", nentries);
        ...
        NR_FF         =	fit_fraction(NR , Opt_Model, {D_MASS, Kminus_MASS, Kplus_MASS}, nentries);
		...
		std::cout << "Sum :"
				  << Phi_12_FF  + Phi_13_FF + f0X_12_FF + f0X_13_FF + f0_12_FF + f0_13_FF  + NR_FF << std::endl;
