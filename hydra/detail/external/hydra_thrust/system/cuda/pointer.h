...
struct iterator_traits<hydra_thrust::cuda_cub::pointer<Element> >
{
private:
  typedef hydra_thrust::cuda_cub::pointer<Element> ptr;
...
{
  typedef hydra_thrust::cuda_cub::reference<Element> type;
...
class pointer
    : public hydra_thrust::pointer<
          T,
          hydra_thrust::cuda_cub::tag,
          hydra_thrust::cuda_cub::reference<T>,
          hydra_thrust::cuda_cub::pointer<T> >
{
...
public:
   ...
   *          to \p hydra_thrust::system::cuda::tag and its element type shall be convertible to \p T.
   */
   ...
   *  \param other The \p OtherPointer to copy.
   *  \tparam OtherPointer The system tag associated with \p OtherPointer shall be convertible
   *          to \p hydra_thrust::system::cuda::tag and its element type shall be \p void.
   */
  template <typename OtherPointer>
  __host__ __device__
  explicit
...
/*! \namespace hydra_thrust::system::cuda
 *  \brief \p hydra_thrust::system::cuda is the namespace containing functionality for allocating, manipulating,
 ...
 *         for import convenience but are also aliased in the top-level <tt>hydra_thrust::cuda</tt>
 *         namespace for easy access.
 *
 */

namespace cuda {
using hydra_thrust::cuda_cub::pointer;
using hydra_thrust::cuda_cub::reference;
} // end cuda
...
#include <hydra/detail/external/hydra_thrust/system/cuda/detail/pointer.inl>
