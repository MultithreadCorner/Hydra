...
// eval_ref<T> is
// - T when T is a subclass of hydra_thrust::reference
// - T& otherwise
// This is used to let hydra_thrust::references pass through actor evaluations.
template <typename T>
using eval_ref = typename std::conditional<
  hydra_thrust::detail::is_wrapped_reference<T>::value, T, T&>::type;
    ...
    : Eval
{
  ...
  __host__ __device__
  typename apply_actor<eval_type, hydra_thrust::tuple<eval_ref<Ts>...>>::type
