 ...
 *  hydra_thrust::default_random_engine g;
 *  hydra_thrust::shuffle(hydra_thrust::host, A, A + N, g);
 ...
 *  const int N = sizeof(A)/sizeof(int);
 *  hydra_thrust::default_random_engine g;
 *  hydra_thrust::shuffle_copy(hydra_thrust::host, A, A + N, result, g);
 ...
 *  const int N = sizeof(A)/sizeof(int);
 *  hydra_thrust::default_random_engine g;
 *  hydra_thrust::shuffle_copy(A, A + N, result, g);
 *  // result is now {6, 5, 8, 7, 2, 1, 4, 3, 10, 9}
 *  \endcode
 *
 *  \see \p shuffle
...
#include <hydra/detail/external/hydra_thrust/detail/shuffle.inl>
#endif
