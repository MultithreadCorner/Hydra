 ...
 *  \p device_vector of integers using the \p hydra_thrust::device execution policy for
 *  parallelization:
 *
 *  \code
 ...
 *  hydra_thrust::device_vector<int> v(data, data + N);
 *  hydra_thrust::reverse(hydra_thrust::device, v.begin(), v.end());
 ...
 *  int data[N] = {0, 1, 2, 3, 4, 5};
 *  hydra_thrust::device_vector<int> v(data, data + N);
 *  hydra_thrust::reverse(v.begin(), v.end());
 *  // v is now {5, 4, 3, 2, 1, 0}
 *  \endcode
 *  
 *  \see https://en.cppreference.com/w/cpp/algorithm/reverse
 ...
 *  an input \p device_vector of integers to an output \p device_vector using the \p hydra_thrust::device
 ...
 *  const int N = 6;
 *  int data[N] = {0, 1, 2, 3, 4, 5};
 *  hydra_thrust::device_vector<int> input(data, data + N);
 *  hydra_thrust::device_vector<int> output(N);
 *  hydra_thrust::reverse_copy(hydra_thrust::device, v.begin(), v.end(), output.begin());
 ...
 *  hydra_thrust::device_vector<int> input(data, data + N);
 *  hydra_thrust::device_vector<int> output(N);
 *  hydra_thrust::reverse_copy(v.begin(), v.end(), output.begin());
 *  // input is still {0, 1, 2, 3, 4, 5}
 *  // output is now  {5, 4, 3, 2, 1, 0}
 *  \endcode
 *  
