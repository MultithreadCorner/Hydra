...
#ifndef HYDRA_BOOST_MATH_TOOLS_BIVARIATE_STATISTICS_HPP
#define HYDRA_BOOST_MATH_TOOLS_BIVARIATE_STATISTICS_HPP
...
HYDRA_BOOST_MATH_HEADER_DEPRECATED("<hydra/detail/external/hydra_boost/math/statistics/bivariate_statistics.hpp>");
...
auto means_and_covariance(Container const & u, Container const & v)
{
    ...
    using std::size;
    HYDRA_BOOST_MATH_ASSERT_MSG(size(u) == size(v), "The size of each vector must be the same to compute covariance.");
    HYDRA_BOOST_MATH_ASSERT_MSG(size(u) > 0, "Computing covariance requires at least one sample.");
...
{
    auto [mu_u, mu_v, cov] = hydra_boost::math::tools::means_and_covariance(u, v);
    return cov;
}
...
auto correlation_coefficient(Container const & u, Container const & v)
{
    ...
    using std::size;
    HYDRA_BOOST_MATH_ASSERT_MSG(size(u) == size(v), "The size of each vector must be the same to compute covariance.");
    HYDRA_BOOST_MATH_ASSERT_MSG(size(u) > 0, "Computing covariance requires at least two samples.");
