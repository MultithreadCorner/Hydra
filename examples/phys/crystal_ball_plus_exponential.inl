...
int main(int argv, char** argc)
{
	...
	//fix tail parameters (usualy done with mc)
	auto  n     = hydra::Parameter::Create().Name("N").Value(2.5).Error(0.0001).Limits(0.0, 5.0).Fixed();

	//Signal PDF
	hydra::CrystalBallShape<> signal(mean, sigma, alpha, n);

	auto Signal_PDF = hydra::make_pdf(signal, hydra::CrystalBallShapeAnalyticalIntegral(min, max));
    ...
    //Background
    hydra::Exponential<> background(tau);

    auto Background_PDF = hydra::make_pdf(background, hydra::ExponentialAnalyticalIntegral(min, max));
