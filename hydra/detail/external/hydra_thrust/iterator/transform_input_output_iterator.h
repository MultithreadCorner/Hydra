...
/*! \file hydra_thrust/iterator/transform_input_output_iterator.h
...
#include <hydra/detail/external/hydra_thrust/iterator/detail/transform_input_output_iterator.inl>
 ...
 *
 * \code
 * #include <hydra/detail/external/hydra_thrust/iterator/transform_input_output_iterator.h>
 * #include <hydra/detail/external/hydra_thrust/device_vector.h>
 *
 *  int main()
 *  {
 *    const size_t size = 4;
 *    hydra_thrust::device_vector<float> v(size);
 *
 *    // Write 1.0f, 2.0f, 3.0f, 4.0f to vector
 *    hydra_thrust::sequence(v.begin(), v.end(), 1);
 *
 *    // Iterator that returns negated values and writes squared values
 *    auto iter = hydra_thrust::make_transform_input_output_iterator(v.begin(),
 *        hydra_thrust::negate<float>{}, hydra_thrust::square<float>{});
 * 
 ...
 *    hydra_thrust::sequence(iter, iter + size, 1);
 *
