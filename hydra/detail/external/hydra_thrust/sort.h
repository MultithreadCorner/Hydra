 ...
 *  a sequence of integers using the \p hydra_thrust::host execution policy for parallelization:
 *
 *  \code
 ...
 *  const int N = 6;
 *  int A[N] = {1, 4, 2, 8, 5, 7};
 *  hydra_thrust::sort(hydra_thrust::host, A, A + N);
 ...
 *  using the greater<int> comparison operator using the \p hydra_thrust::host execution policy for parallelization:
 *
 *  \code
 *  #include <hydra/detail/external/hydra_thrust/sort.h>
 ...
 *  const int N = 6;
 *  int A[N] = {1, 4, 2, 8, 5, 7};
 *  hydra_thrust::sort(hydra_thrust::host, A, A + N, hydra_thrust::greater<int>());
 ...
 *  const int N = 6;
 *  int A[N] = {1, 4, 2, 8, 5, 7};
 *  hydra_thrust::sort(A, A + N, hydra_thrust::greater<int>());
 *  // A is now {8, 7, 5, 4, 2, 1};
 *  \endcode
 *
 *  \see https://en.cppreference.com/w/cpp/algorithm/sort
 ...
 *  const int N = 6;
 *  int A[N] = {1, 4, 2, 8, 5, 7};
 *  hydra_thrust::stable_sort(hydra_thrust::host, A, A + N);
 *  // A is now {1, 2, 4, 5, 7, 8}
 *  \endcode
 *
 ...
 *  const int N = 6;
 *  int A[N] = {1, 4, 2, 8, 5, 7};
 *  hydra_thrust::stable_sort(A, A + N);
 *  // A is now {1, 2, 4, 5, 7, 8}
 *  \endcode
 *
 *  \see https://en.cppreference.com/w/cpp/algorithm/stable_sort
 ...
 *  an array of character values using integers as sorting keys using the \p hydra_thrust::host execution policy
 *  for parallelization:
 *
 *  \code
 *  #include <hydra/detail/external/hydra_thrust/sort.h>
 ...
 *  hydra_thrust::sort_by_key(hydra_thrust::host, keys, keys + N, values);
 ...
 *  int    keys[N] = {  1,   4,   2,   8,   5,   7};
 *  char values[N] = {'a', 'b', 'c', 'd', 'e', 'f'};
 *  hydra_thrust::sort_by_key(keys, keys + N, values);
 *  // keys is now   {  1,   2,   4,   5,   7,   8}
 *  // values is now {'a', 'c', 'b', 'e', 'f', 'd'}
 *  \endcode
 *
 ...
 *  int    keys[N] = {  1,   4,   2,   8,   5,   7};
 *  char values[N] = {'a', 'b', 'c', 'd', 'e', 'f'};
 *  hydra_thrust::sort_by_key(hydra_thrust::host, keys, keys + N, values, hydra_thrust::greater<int>());
 ...
 *  int    keys[N] = {  1,   4,   2,   8,   5,   7};
 *  char values[N] = {'a', 'b', 'c', 'd', 'e', 'f'};
 *  hydra_thrust::sort_by_key(keys, keys + N, values, hydra_thrust::greater<int>());
 *  // keys is now   {  8,   7,   5,   4,   2,   1}
 *  // values is now {'d', 'f', 'e', 'b', 'c', 'a'}
 *  \endcode
 *
 ...
 *  an array of characters using integers as sorting keys using the \p hydra_thrust::host execution policy for
 *  parallelization:
 *
 *  \code
 *  #include <hydra/detail/external/hydra_thrust/sort.h>
 ...
 *  int    keys[N] = {  1,   4,   2,   8,   5,   7};
 *  char values[N] = {'a', 'b', 'c', 'd', 'e', 'f'};
 *  hydra_thrust::stable_sort_by_key(hydra_thrust::host, keys, keys + N, values);
 *  // keys is now   {  1,   2,   4,   5,   7,   8}
 *  // values is now {'a', 'c', 'b', 'e', 'f', 'd'}
 *  \endcode
 *
 ...
 *  int    keys[N] = {  1,   4,   2,   8,   5,   7};
 *  char values[N] = {'a', 'b', 'c', 'd', 'e', 'f'};
 *  hydra_thrust::stable_sort_by_key(keys, keys + N, values);
 *  // keys is now   {  1,   2,   4,   5,   7,   8}
 *  // values is now {'a', 'c', 'b', 'e', 'f', 'd'}
 *  \endcode
 *
 ...
 *
 *  The following code snippet demonstrates how to use \p sort_by_key to sort
 *  an array of character values using integers as sorting keys using the \p hydra_thrust::host execution policy for
 ...
 *  int    keys[N] = {  1,   4,   2,   8,   5,   7};
 *  char values[N] = {'a', 'b', 'c', 'd', 'e', 'f'};
 *  hydra_thrust::stable_sort_by_key(hydra_thrust::host, keys, keys + N, values, hydra_thrust::greater<int>());
 *  // keys is now   {  8,   7,   5,   4,   2,   1}
 *  // values is now {'d', 'f', 'e', 'b', 'c', 'a'}
 *  \endcode
 *
 ...
 *  int    keys[N] = {  1,   4,   2,   8,   5,   7};
 *  char values[N] = {'a', 'b', 'c', 'd', 'e', 'f'};
 *  hydra_thrust::stable_sort_by_key(keys, keys + N, values, hydra_thrust::greater<int>());
 *  // keys is now   {  8,   7,   5,   4,   2,   1}
 *  // values is now {'d', 'f', 'e', 'b', 'c', 'a'}
 *  \endcode
 *
 ...
 *  contents of a \c device_vector are stored in ascending order using the \p hydra_thrust::device execution policy
 *  for parallelization:
 *
 *  \code
 *  #include <hydra/detail/external/hydra_thrust/sort.h>
 ...
 *  #include <hydra/detail/external/hydra_thrust/execution_policy.h>
 *  ...
 *  hydra_thrust::device_vector<int> v(6);
 *  v[0] = 1;
 *  v[1] = 4;
 *  v[2] = 2;
 *  v[3] = 8;
 ...
 *  v[5] = 7;
 *
 *  bool result = hydra_thrust::is_sorted(hydra_thrust::device, v.begin(), v.end());
 *
 *  // result == false
 *
 *  hydra_thrust::sort(v.begin(), v.end());
 *  result = hydra_thrust::is_sorted(hydra_thrust::device, v.begin(), v.end());
 *
 *  // result == true
 *  \endcode
 *
 ...
 *  v[5] = 7;
 *
 *  bool result = hydra_thrust::is_sorted(v.begin(), v.end());
 *
 *  // result == false
 *
 *  hydra_thrust::sort(v.begin(), v.end());
 *  result = hydra_thrust::is_sorted(v.begin(), v.end());
 *
 *  // result == true
 *  \endcode
 *
 ...
 *  contents of a \c device_vector are stored in descending order using the \p hydra_thrust::device execution
 ...
 *  v[5] = 7;
 *
 *  hydra_thrust::greater<int> comp;
 *  bool result = hydra_thrust::is_sorted(hydra_thrust::device, v.begin(), v.end(), comp);
 *
 *  // result == false
 *
 *  hydra_thrust::sort(v.begin(), v.end(), comp);
 *  result = hydra_thrust::is_sorted(hydra_thrust::device, v.begin(), v.end(), comp);
 *
 *  // result == true
 *  \endcode
 *
 ...
 *
 *  hydra_thrust::greater<int> comp;
 *  bool result = hydra_thrust::is_sorted(v.begin(), v.end(), comp);
 *
 *  // result == false
 *
 *  hydra_thrust::sort(v.begin(), v.end(), comp);
 *  result = hydra_thrust::is_sorted(v.begin(), v.end(), comp);
 *
 *  // result == true
 *  \endcode
 *
 ...
 *  in an array where the data becomes unsorted using the \p hydra_thrust::host execution policy for
 *  parallelization:
 *  
 *  \code
 *  #include <hydra/detail/external/hydra_thrust/sort.h>
 ...
 *  
 *  int * B = hydra_thrust::is_sorted_until(hydra_thrust::host, A, A + 8);
 *  
 *  // B - A is 4
 *  // [A, B) is sorted
 *  \endcode
 ...
 *  int A[8] = {0, 1, 2, 3, 0, 1, 2, 3};
 *  
 *  int * B = hydra_thrust::is_sorted_until(A, A + 8);
 *  
 *  // B - A is 4
 *  // [A, B) is sorted
 *  \endcode
 ...
 *
 *  The following code snippet demonstrates how to use \p is_sorted_until to find the first position
 *  in an array where the data becomes unsorted in descending order using the \p hydra_thrust::host execution
 *  policy for parallelization:
 *
 *  \code
 *  #include <hydra/detail/external/hydra_thrust/sort.h>
 ...
 *  
 *  hydra_thrust::greater<int> comp;
 *  int * B = hydra_thrust::is_sorted_until(hydra_thrust::host, A, A + 8, comp);
 *  
 *  // B - A is 4
 ...
 *  
 *  hydra_thrust::greater<int> comp;
 *  int * B = hydra_thrust::is_sorted_until(A, A + 8, comp);
 *  
 *  // B - A is 4
 *  // [A, B) is sorted in descending order
 *  \endcode
...
#include <hydra/detail/external/hydra_thrust/detail/sort.inl>
