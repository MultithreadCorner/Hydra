...
/*! \file hydra_thrust/iterator/reverse_iterator.h
 ...
 *  v[3] = 3.0f;
 *
 *  typedef hydra_thrust::device_vector<float>::iterator Iterator;
 *
 *  // note that we point the iterator to the *end* of the device_vector
 *  hydra_thrust::reverse_iterator<Iterator> iter(values.end());
 *
 *  *iter;   // returns 3.0f;
 *  iter[0]; // returns 3.0f;
 *  iter[1]; // returns 2.0f;
 ...
 *  hydra_thrust::device_iterator<float>::reverse_iterator iter = values.rbegin();
 *
 *  *iter;   // returns 3.0f;
 *  iter[0]; // returns 3.0f;
 *  iter[1]; // returns 2.0f;
 ...
 *  ...
 *  hydra_thrust::device_vector<int> v(5);
 *  v[0] = 0;
 *  v[1] = 1;
 *  v[2] = 2;
 *  v[3] = 3;
 *  v[4] = 4;
 *
 *  hydra_thrust::device_vector<int> result(5);
 *
 *  // exclusive scan v into result in reverse
 *  hydra_thrust::exclusive_scan(v.rbegin(), v.rend(), result.begin());
 *
