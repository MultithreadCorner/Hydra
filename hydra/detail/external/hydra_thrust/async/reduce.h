...
#include <hydra/detail/external/hydra_thrust/system/detail/adl/async/reduce.h>
...
using hydra_thrust::async::unimplemented::async_reduce;
...
struct reduce_fn final
{
  ...
  HYDRA_THRUST_RETURNS(
    ...
    , HYDRA_THRUST_FWD(first), HYDRA_THRUST_FWD(last)
    , HYDRA_THRUST_FWD(init)
    , hydra_thrust::plus<remove_cvref_t<T>>{}
    )
  )
...
using hydra_thrust::async::unimplemented::async_reduce_into;
