...
/*! \file hydra_thrust/iterator/zip_iterator.h
 ...
 *  ...
 *  hydra_thrust::device_vector<int> int_v(3);
 ...
 *  typedef hydra_thrust::device_vector<int>::iterator   IntIterator;
 *  typedef hydra_thrust::device_vector<float>::iterator FloatIterator;
 *  typedef hydra_thrust::device_vector<char>::iterator  CharIterator;
 *
 *  // typedef a tuple of these iterators
 *  typedef hydra_thrust::tuple<IntIterator, FloatIterator, CharIterator> IteratorTuple;
 *
 *  // typedef the zip_iterator of this tuple
 *  typedef hydra_thrust::zip_iterator<IteratorTuple> ZipIterator;
 *
 *  // finally, create the zip_iterator
 *  ZipIterator iter(hydra_thrust::make_tuple(int_v.begin(), float_v.begin(), char_v.begin()));
 *
 ...
 *
 *  hydra_thrust::get<0>(iter[2]); // returns 2
 *  hydra_thrust::get<1>(iter[0]); // returns 0.0f
 *  hydra_thrust::get<2>(iter[1]); // returns 'b'
 *
 ...
 *  \p zip_iterator to copy multiple ranges with a single call to \p hydra_thrust::copy.
 *
 *  \code
 *  #include <hydra/detail/external/hydra_thrust/zip_iterator.h>
 *  #include <hydra/detail/external/hydra_thrust/tuple.h>
 *  #include <hydra/detail/external/hydra_thrust/device_vector.h>
 *
 *  int main()
 ...
 *    int_in[2] = 2;
 *
 *    hydra_thrust::device_vector<float> float_in(3), float_out(3);
 *    float_in[0] =  0.0f;
 *    float_in[1] = 10.0f;
 *    float_in[2] = 20.0f;
 *
 *    hydra_thrust::copy(hydra_thrust::make_zip_iterator(hydra_thrust::make_tuple(int_in.begin(), float_in.begin())),
 *                 hydra_thrust::make_zip_iterator(hydra_thrust::make_tuple(int_in.end(),   float_in.end())),
 *                 hydra_thrust::make_zip_iterator(hydra_thrust::make_tuple(int_out.begin(),float_out.begin())));
 *
