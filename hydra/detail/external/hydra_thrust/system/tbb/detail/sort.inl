...
void merge_sort(execution_policy<DerivedPolicy> &exec, Iterator1 first1, Iterator1 last1, Iterator2 first2, StrictWeakOrdering comp, bool inplace)
{
  ...
  if(inplace) hydra_thrust::merge(exec, first2, mid2, mid2, last2, first1, comp);
  else	      hydra_thrust::merge(exec, first1, mid1, mid1, last1, first2, comp);
}
                       ...
                       bool inplace)
{
  ...
  if (n < threshold)
  {
    ...
    if(!inplace)
    {
      ...
      hydra_thrust::copy(hydra_thrust::seq, first2, last2, first4);
    }
  ...
  if(inplace)
  {
    hydra_thrust::merge_by_key(exec, first3, mid3, mid3, last3, first4, mid4, first1, first2, comp);
  }
  else
  {
    hydra_thrust::merge_by_key(exec, first1, mid1, mid1, last1, first2, mid2, first3, first4, comp);
  }
}
...
void stable_sort(execution_policy<DerivedPolicy> &exec,
                 ...
                 StrictWeakOrdering comp)
{
  typedef typename hydra_thrust::iterator_value<RandomAccessIterator>::type key_type;

  hydra_thrust::detail::temporary_array<key_type, DerivedPolicy> temp(exec, first, last);
...
template<typename DerivedPolicy,
  ...
  void stable_sort_by_key(execution_policy<DerivedPolicy> &exec,
                          ...
                          StrictWeakOrdering comp)
{
  typedef typename hydra_thrust::iterator_value<RandomAccessIterator1>::type key_type;
  typedef typename hydra_thrust::iterator_value<RandomAccessIterator2>::type val_type;

  RandomAccessIterator2 last2 = first2 + hydra_thrust::distance(first1, last1);

  hydra_thrust::detail::temporary_array<key_type, DerivedPolicy> temp1(exec, first1, last1);
  hydra_thrust::detail::temporary_array<val_type, DerivedPolicy> temp2(exec, first2, last2);
