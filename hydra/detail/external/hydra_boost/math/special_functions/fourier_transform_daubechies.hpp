...
#ifndef HYDRA_BOOST_MATH_SPECIAL_FOURIER_TRANSFORM_DAUBECHIES_HPP
#define HYDRA_BOOST_MATH_SPECIAL_FOURIER_TRANSFORM_DAUBECHIES_HPP
#include <array>
#include <cmath>
#include <complex>
#include <iostream>
...
#include <hydra/detail/external/hydra_boost/math/tools/estrin.hpp>
...
template <typename Real, unsigned N> constexpr std::array<Real, N> ft_daubechies_scaling_polynomial_coefficients() {
  ...
  if constexpr (N == 2) {
    return {HYDRA_BOOST_MATH_BIG_CONSTANT(Real, std::numeric_limits<Real>::digits,
                                    1.36602540378443864676372317075293618347140262690519031402790348972596650842632007803393058),
            HYDRA_BOOST_MATH_BIG_CONSTANT(Real, std::numeric_limits<Real>::digits,
...
template <class Real, unsigned p> std::complex<Real> fourier_transform_daubechies_scaling(Real omega) {
  ...
  using hydra_boost::math::constants::one_div_root_two_pi;
  using std::abs;
  using std::exp;
  using std::norm;
  using std::pow;
  ...
  do {
    ...
    auto z = exp(arg);
    phi *= hydra_boost::math::tools::evaluate_polynomial_estrin(lxi, z);
    xi /= 2;
...
template <class Real, unsigned p> std::complex<Real> fourier_transform_daubechies_wavelet(Real omega) {
  ...
  //auto z = exp(std::complex<Real>(0, -omega/2 - hydra_boost::math::constants::pi<Real>()));
  auto constexpr lxi = detail::ft_daubechies_scaling_polynomial_coefficients<Real, p>();
  auto m0 = std::complex<Real>(pow((Real(1) + z)/Real(2), p))*hydra_boost::math::tools::evaluate_polynomial_estrin(lxi, z);
