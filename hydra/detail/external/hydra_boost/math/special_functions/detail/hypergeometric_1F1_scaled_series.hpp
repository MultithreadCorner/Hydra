...
//
#ifndef HYDRA_BOOST_MATH_HYPERGEOMETRIC_1F1_SCALED_SERIES_HPP
#define HYDRA_BOOST_MATH_HYPERGEOMETRIC_1F1_SCALED_SERIES_HPP
...
#include <cstdint>
  ...
  namespace hydra_boost{ namespace math{ namespace detail{
     ...
     T hypergeometric_1F1_scaled_series(const T& a, const T& b, T z, const Policy& pol, const char* function)
     {
        ...
        unsigned n = 0;
        long long log_scaling_factor = 1 - lltrunc(hydra_boost::math::tools::log_max_value<T>());
        ...
        do
        {
           ...
           ++n;
           diff = fabs(term / sum);
        } while (diff > hydra_boost::math::policies::get_epsilon<T, Policy>());
...
#endif // HYDRA_BOOST_MATH_HYPERGEOMETRIC_1F1_SCALED_SERIES_HPP
