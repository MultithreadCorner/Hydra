...
#ifndef MULTIARRAY2_H_
#define MULTIARRAY2_H_
...
public:
	...
	multiarray2(size_t n) { __resize(n); };

	multiarray2(size_t n, value_type const& value) {
		__resize(n);
	...
	multiarray2(hydra::pair<Int, typename detail::tuple_type<N, T>::type > const& pair){
		__resize(pair.first);
		HYDRA_EXTERNAL_NS::thrust::fill(begin(), end(), pair.second );
	}


	multiarray2(multiarray2<N,T,detail::BackendPolicy<BACKEND>> const& other )
	{
		__resize( other.size() );
		HYDRA_EXTERNAL_NS::thrust::copy(other.begin(), other.end(), begin());
	}

	multiarray2(multiarray2<N,T,detail::BackendPolicy<BACKEND>>&& other ):
	fData(other.__move())
	{}

	template< hydra::detail::Backend BACKEND2>
	multiarray2(multiarray2<N,T,detail::BackendPolicy<BACKEND2>> const& other )
	{
		__resize(other.size());
		HYDRA_EXTERNAL_NS::thrust::copy(other.begin(), other.end(), begin());
	}
	...
	multiarray2(Iterator first, Iterator last )
	{
	...
	multiarray2<N,T,detail::BackendPolicy<BACKEND>>&
	operator=(multiarray2<N,T,detail::BackendPolicy<BACKEND>> const& other )
	{
	...
	multiarray2<N,T,detail::BackendPolicy<BACKEND>>&
	operator=(multiarray2<N,T,detail::BackendPolicy<BACKEND> >&& other )
	{
		if(this == &other) return *this;
	...
	template< hydra::detail::Backend BACKEND2>
	multiarray2<N,T,detail::BackendPolicy<BACKEND> >&
	operator=(multiarray2<N,T,detail::BackendPolicy<BACKEND2> > const& other )
	{
		HYDRA_EXTERNAL_NS::thrust::copy(other.begin(), other.end(), begin());
		return *this;
	}
