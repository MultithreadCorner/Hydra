...
public:
  ...
  /** \returns the distance of a point \a p to its projection onto the line \c *this.
    * \sa squaredDistance()
    */
  HYDRA_EIGEN_DEVICE_FUNC RealScalar distance(const VectorType& p) const { HYDRA_EIGEN_USING_STD(sqrt) return sqrt(squaredDistance(p)); }
  ...
  HYDRA_EIGEN_DEVICE_FUNC inline ParametrizedLine& transform(const MatrixBase<XprType>& mat, TransformTraits traits = Affine)
  {
    if (traits==Affine)
  ...
  HYDRA_EIGEN_DEVICE_FUNC inline ParametrizedLine& transform(const Transform<Scalar,AmbientDimAtCompileTime,Affine,TrOptions>& t,
