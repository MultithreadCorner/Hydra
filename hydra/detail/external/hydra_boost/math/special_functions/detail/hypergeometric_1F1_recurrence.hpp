...
#ifndef HYDRA_BOOST_HYPERGEOMETRIC_1F1_RECURRENCE_HPP_
#define HYDRA_BOOST_HYPERGEOMETRIC_1F1_RECURRENCE_HPP_
...
#include <hydra/detail/external/hydra_boost/math/special_functions/detail/hypergeometric_pFq_checked_series.hpp>
  ...
  {
    using result_type = hydra_boost::math::tuple<T, T, T>;
    ...
    result_type operator()(std::intmax_t i) const
    {
      ...
      return hydra_boost::math::make_tuple(an, bn, cn);
    }
  ...
  {
     typedef hydra_boost::math::tuple<T, T, T> result_type;
  ...
  {
    HYDRA_BOOST_MATH_STD_USING // modf, frexp, fabs, pow
  ...
  T hypergeometric_1F1_backwards_recursion_on_b_for_negative_a(const T& a, const T& b, const T& z, const Policy& pol, const char*, long long& log_scaling)
  {
     ...
     HYDRA_BOOST_MATH_ASSERT(a < -1); // Not tested nor taken for -1 < a < 0
     ...
     //
     if (b_shift > static_cast<std::intmax_t>(hydra_boost::math::policies::get_max_series_iterations<Policy>()))
        return hypergeometric_1F1_checked_series_impl(a, b, z, pol, log_scaling);

     if (a_shift > static_cast<std::intmax_t>(hydra_boost::math::policies::get_max_series_iterations<Policy>()))
        return hypergeometric_1F1_checked_series_impl(a, b, z, pol, log_scaling);
     ...
     HYDRA_BOOST_MATH_ASSERT(leading_a_shift > 1);
     HYDRA_BOOST_MATH_ASSERT(a_b_shift + leading_a_shift + (a_b_shift == 0 ? 1 : 0) == a_shift);
     HYDRA_BOOST_MATH_ASSERT(a_b_shift + trailing_b_shift == b_shift);
     ...
     long long scale1 {};
     long long scale2 {};
     first = hydra_boost::math::detail::hypergeometric_1F1_imp(T(a + a_shift), T(b + b_shift), z, pol, scale1);
     //
     ...
     //
     second = hydra_boost::math::detail::hypergeometric_1F1_imp(T(a + a_shift - 1), T(b + b_shift), z, pol, scale2);
     if (scale1 != scale2)
     ...
     //
     second = hydra_boost::math::tools::apply_recurrence_relation_backward(
...
#endif // HYDRA_BOOST_HYPERGEOMETRIC_1F1_RECURRENCE_HPP_
