 ...
 *  then \p hydra_thrust::reduce should not be used.  Instead, one could use 
 ...
 *  the sum of a sequence of integers using the \p hydra_thrust::host execution policy for parallelization:
 *
 *  \code
 ...
 *  int result = hydra_thrust::reduce(hydra_thrust::host, data, data + 6);
 *
 *  // result == 9
 *  \endcode
 *
 ...
 *  ...
 *  int data[6] = {1, 0, 2, 2, 1, 3};
 *  int result = hydra_thrust::reduce(data, data + 6);
 *
 *  // result == 9
 *  \endcode
 *
...
template<typename InputIterator> typename
  hydra_thrust::iterator_traits<InputIterator>::value_type reduce(InputIterator first, InputIterator last);
 ...
 *
 *  The following code snippet demonstrates how to use \p reduce to compute
 *  the sum of a sequence of integers including an intialization value using the \p hydra_thrust::host
 ...
 *  ...
 *  int data[6] = {1, 0, 2, 2, 1, 3};
 *  int result = hydra_thrust::reduce(hydra_thrust::host, data, data + 6, 1);
 *
 *  // result == 10
 *  \endcode
 *
 ...
 *  ...
 *  int data[6] = {1, 0, 2, 2, 1, 3};
 *  int result = hydra_thrust::reduce(data, data + 6, 1);
 *
 *  // result == 10
 *  \endcode
 *
 ...
 *  compute the maximum value of a sequence of integers using the \p hydra_thrust::host execution policy
 *  for parallelization:
 *
 *  \code
 *  #include <hydra/detail/external/hydra_thrust/reduce.h>
 ...
 *  ...
 *  int data[6] = {1, 0, 2, 2, 1, 3};
 *  int result = hydra_thrust::reduce(hydra_thrust::host,
 *                              data, data + 6,
 *                              -1,
 *                              hydra_thrust::maximum<int>());
 *  // result == 3
 *  \endcode
 *
 *  \see https://en.cppreference.com/w/cpp/algorithm/accumulate
 ...
 *  compact a sequence of key/value pairs and sum values with equal keys using the \p hydra_thrust::host
 *  execution policy for parallelization:
 *
 *  \code
 *  #include <hydra/detail/external/hydra_thrust/reduce.h>
 ...
 *  new_end = hydra_thrust::reduce_by_key(hydra_thrust::host, A, A + N, B, C, D);
 *
 ...
 *
 *  hydra_thrust::pair<int*,int*> new_end;
 *  new_end = hydra_thrust::reduce_by_key(A, A + N, B, C, D);
 *
 *  // The first four keys in C are now {1, 3, 2, 1} and new_end.first - C is 4.
 *  // The first four values in D are now {9, 21, 9, 3} and new_end.second - D is 4.
 *  \endcode
 ...
 *  new_end = hydra_thrust::reduce_by_key(hydra_thrust::host, A, A + N, B, C, D, binary_pred);
 *
 *  // The first four keys in C are now {1, 3, 2, 1} and new_end.first - C is 4.
 *  // The first four values in D are now {9, 21, 9, 3} and new_end.second - D is 4.
 *  \endcode
 ...
 *  hydra_thrust::pair<int*,int*> new_end;
 *  hydra_thrust::equal_to<int> binary_pred;
 *  new_end = hydra_thrust::reduce_by_key(A, A + N, B, C, D, binary_pred);
 *
 *  // The first four keys in C are now {1, 3, 2, 1} and new_end.first - C is 4.
 *  // The first four values in D are now {9, 21, 9, 3} and new_end.second - D is 4.
 *  \endcode
 ...
 *  new_end = hydra_thrust::reduce_by_key(hydra_thrust::host, A, A + N, B, C, D, binary_pred, binary_op);
 *
 *  // The first four keys in C are now {1, 3, 2, 1} and new_end.first - C is 4.
 *  // The first four values in D are now {9, 21, 9, 3} and new_end.second - D is 4.
 *  \endcode
 ...
 *  hydra_thrust::equal_to<int> binary_pred;
 *  hydra_thrust::plus<int> binary_op;
 *  new_end = hydra_thrust::reduce_by_key(A, A + N, B, C, D, binary_pred, binary_op);
 *
 *  // The first four keys in C are now {1, 3, 2, 1} and new_end.first - C is 4.
 *  // The first four values in D are now {9, 21, 9, 3} and new_end.second - D is 4.
 *  \endcode
...
#include <hydra/detail/external/hydra_thrust/detail/reduce.inl>
