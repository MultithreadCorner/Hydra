...
#include <hydra/detail/external/thrust/detail/functional/value.h>
  ...
  struct as_actor
{
  ...
  static inline __hydra_host__ __hydra_device__ type convert(const T &x)
  {
    return val(x);
  } // end convert()
}; // end as_actor
  ...
  struct as_actor<actor<Eval> >
{
  ...
  static inline __hydra_host__ __hydra_device__ const type &convert(const actor<Eval> &x)
  {
    return x;
  } // end convert()
}; // end as_actor
