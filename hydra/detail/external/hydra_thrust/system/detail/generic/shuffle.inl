...
#include <hydra/detail/external/hydra_thrust/system/detail/generic/shuffle.h>
...
__host__ __device__ void shuffle(
    hydra_thrust::execution_policy<ExecutionPolicy>& exec, RandomIterator first,
    RandomIterator last, URBG&& g) {
  using InputType = typename hydra_thrust::iterator_value_t<RandomIterator>;

  // copy input to temp buffer
  hydra_thrust::detail::temporary_array<InputType, ExecutionPolicy> temp(exec, first,
                                                                   last);
  hydra_thrust::shuffle_copy(exec, temp.begin(), temp.end(), first, g);
}
...
__host__ __device__ void shuffle_copy(
  ...
  hydra_thrust::counting_iterator<std::uint64_t> indices(0);
  hydra_thrust::transform_iterator<construct_key_flag_op, decltype(indices),
                             key_flag_tuple>
  ...
  write_output_op<RandomIterator, decltype(result)> write_functor{m, first,
                                                                  result};
  auto gather_output_it = hydra_thrust::make_transform_output_iterator(
      hydra_thrust::discard_iterator<std::size_t>(), write_functor);
  ...
  hydra_thrust::inclusive_scan(exec, key_flag_it, key_flag_it + n, gather_output_it,
                         key_flag_scan_op());
}
