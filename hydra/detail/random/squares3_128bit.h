...
#ifndef SQUARES3_128BIT_H_
#define SQUARES3_128BIT_H_
...
#include <hydra/detail/random/detail/uint128.h>
...
public:
	...
	squares3_128bit()=delete;

	__hydra_host__ __hydra_device__
	squares3_128bit(uint64_t  s):
	  fState(0),
      fSeed(seed_type{ splitmix<uint64_t>(s) })
    {}

	__hydra_host__ __hydra_device__
	squares3_128bit( squares3_128bit const& other):
	  fState(other.GetState() ),
      fSeed(other.GetSeed() )
     {}

	__hydra_host__ __hydra_device__
	inline squares3_128bit& operator=( squares3_128bit const& other)
	{
	...
	inline result_type operator()(void)
	{
		...
		x = x*x + y; x = (x>>64) | (x<<64);       /* round 1 */

		x = x*x + z; x = (x>>64) | (x<<64);       /* round 2 */

		++fState;                                 /* advance state */

		return uint128_t::u128_to_u64((x*x + y) >> 64);                   /* round 3 */

	}

	__hydra_host__ __hydra_device__
	inline void discard(advance_type n){
		//std::cout << "A -- State:" <<  fState.hi <<" n:"<< n << std::endl;
		fState += n;// uint128_t::add128(fState, n);
		//std::cout << "D -- State:" <<  fState.hi <<" n:"<< n<< std::endl;
		return;
	}
