...
/*! In \c hydra_thrust, the function \c hydra_thrust::device_new allocates memory for
 ...
 *  a range of uninitialized memory using the \p hydra_thrust::device execution policy for
 *  parallelization:
 *
 *  \code
 ...
 *
 *  Int val(46);
 *  hydra_thrust::device_vector<Int> input(N, val);
 *  hydra_thrust::device_ptr<Int> array = hydra_thrust::device_malloc<Int>(N);
 *  hydra_thrust::uninitialized_copy(hydra_thrust::device, input.begin(), input.end(), array);
 *
 *  // Int x = array[i];
 ...
 *  hydra_thrust::device_vector<Int> input(N, val);
 *  hydra_thrust::device_ptr<Int> array = hydra_thrust::device_malloc<Int>(N);
 *  hydra_thrust::uninitialized_copy(input.begin(), input.end(), array);
 *
 *  // Int x = array[i];
 *  // x.val == 46 for all 0 <= i < N
 *  \endcode
 ...
 *  hydra_thrust::device_vector<Int> input(N, val);
 *  hydra_thrust::device_ptr<Int> array = hydra_thrust::device_malloc<Int>(N);
 *  hydra_thrust::uninitialized_copy_n(hydra_thrust::device, input.begin(), N, array);
 *
 *  // Int x = array[i];
 *  // x.val == 46 for all 0 <= i < N
 *  \endcode
 ...
 *  hydra_thrust::device_vector<Int> input(N, val);
 *  hydra_thrust::device_ptr<Int> array = hydra_thrust::device_malloc<Int>(N);
 *  hydra_thrust::uninitialized_copy_n(input.begin(), N, array);
 *
 *  // Int x = array[i];
 *  // x.val == 46 for all 0 <= i < N
 *  \endcode
