...
template <typename T>
struct RadixEncoder : public hydra_thrust::identity<T>
{};


template <>
struct RadixEncoder<char> : public hydra_thrust::unary_function<char, unsigned char>
{
  __host__ __device__
...
template <>
struct RadixEncoder<signed char> : public hydra_thrust::unary_function<signed char, unsigned char>
{
  __host__ __device__
...
template <>
struct RadixEncoder<short> : public hydra_thrust::unary_function<short, unsigned short>
{
  __host__ __device__
...
template <>
struct RadixEncoder<int> : public hydra_thrust::unary_function<int, unsigned int>
{
  __host__ __device__
...
template <>
struct RadixEncoder<long> : public hydra_thrust::unary_function<long, unsigned long>
{
  __host__ __device__
  unsigned long operator()(long x) const
  {
...
template <>
struct RadixEncoder<long long> : public hydra_thrust::unary_function<long long, unsigned long long>
{
  __host__ __device__
...
template <>
struct RadixEncoder<float> : public hydra_thrust::unary_function<float, hydra_thrust::detail::uint32_t>
{
  __host__ __device__
  hydra_thrust::detail::uint32_t operator()(float x) const
  {
    union { float f; hydra_thrust::detail::uint32_t i; } u;
    u.f = x;
    hydra_thrust::detail::uint32_t mask = -static_cast<hydra_thrust::detail::int32_t>(u.i >> 31) | (static_cast<hydra_thrust::detail::uint32_t>(1) << 31);
    return u.i ^ mask;
  }
};

template <>
struct RadixEncoder<double> : public hydra_thrust::unary_function<double, hydra_thrust::detail::uint64_t>
{
  __host__ __device__
  hydra_thrust::detail::uint64_t operator()(double x) const
  {
    union { double f; hydra_thrust::detail::uint64_t i; } u;
    u.f = x;
    hydra_thrust::detail::uint64_t mask = -static_cast<hydra_thrust::detail::int64_t>(u.i >> 63) | (static_cast<hydra_thrust::detail::uint64_t>(1) << 63);
    return u.i ^ mask;
  }
};
...
void radix_shuffle_n(sequential::execution_policy<DerivedPolicy> &exec,
                     ...
                     size_t *histogram)
{
  typedef typename hydra_thrust::iterator_value<RandomAccessIterator1>::type KeyType;

  // note that we are going to mutate the histogram during this sequential scatter
  hydra_thrust::scatter(exec,
                  first, first + n,
                  hydra_thrust::make_transform_iterator(first, bucket_functor<RadixBits,KeyType>(bit_shift, histogram)),
                  result);
}
                     ...
                     size_t *histogram)
{
  ...
  hydra_thrust::scatter(exec,
                  ...
                  hydra_thrust::make_zip_iterator(hydra_thrust::make_tuple(keys_first + n, values_first + n)),
                  hydra_thrust::make_transform_iterator(keys_first, bucket_functor<RadixBits,KeyType>(bit_shift, histogram)),
                  hydra_thrust::make_zip_iterator(hydra_thrust::make_tuple(keys_result, values_result)));
}
                ...
                const size_t N)
{
  ...
  if(flip)
  {
    hydra_thrust::copy(exec, keys2, keys2 + N, keys1);

    if(HasValues)
    {
      hydra_thrust::copy(exec, vals2, vals2 + N, vals1);
    }
  }
}
...
{
  typedef typename hydra_thrust::iterator_value<RandomAccessIterator>::type KeyType;

  size_t N = last - first;

  hydra_thrust::detail::temporary_array<KeyType, DerivedPolicy> temp(exec, N);
...
{
  typedef typename hydra_thrust::iterator_value<RandomAccessIterator1>::type KeyType;
  typedef typename hydra_thrust::iterator_value<RandomAccessIterator2>::type ValueType;

  size_t N = last1 - first1;

  hydra_thrust::detail::temporary_array<KeyType, DerivedPolicy>   temp1(exec, N);
  hydra_thrust::detail::temporary_array<ValueType, DerivedPolicy> temp2(exec, N);
