...
template<typename T> struct avoid_initialization : hydra_thrust::detail::has_trivial_copy_constructor<T> {};
...
__host__ __device__
  temporary_array<T,System>
    ::temporary_array(int, hydra_thrust::execution_policy<System> &system, size_type n)
      :super_t(n, alloc_type(temporary_allocator<T,System>(system)))
{
  ...
  __host__ __device__
    temporary_array<T,System>
      ::temporary_array(hydra_thrust::execution_policy<System> &system,
        ...
        : super_t(alloc_type(temporary_allocator<T,System>(system)))
{
  super_t::allocate(hydra_thrust::distance(first,last));
