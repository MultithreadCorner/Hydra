 ...
 */
#define HYDRA_THRUST_INDEX_TYPE_DISPATCH(status, call, count, arguments) \
    if (count <= hydra_thrust::detail::integer_traits<hydra_thrust::detail::int32_t>::const_max) { \
        auto HYDRA_THRUST_PP_CAT2(count, _fixed) = static_cast<hydra_thrust::detail::int32_t>(count); \
        status = call arguments; \
    } \
    else { \
        auto HYDRA_THRUST_PP_CAT2(count, _fixed) = static_cast<hydra_thrust::detail::int64_t>(count); \
        status = call arguments; \
    }
 ...
 */
#define HYDRA_THRUST_DOUBLE_INDEX_TYPE_DISPATCH(status, call, count1, count2, arguments) \
    if (count1 + count2 <= hydra_thrust::detail::integer_traits<hydra_thrust::detail::int32_t>::const_max) { \
        auto HYDRA_THRUST_PP_CAT2(count1, _fixed) = static_cast<hydra_thrust::detail::int32_t>(count1); \
        auto HYDRA_THRUST_PP_CAT2(count2, _fixed) = static_cast<hydra_thrust::detail::int32_t>(count2); \
        status = call arguments; \
    } \
    else { \
        auto HYDRA_THRUST_PP_CAT2(count1, _fixed) = static_cast<hydra_thrust::detail::int64_t>(count1); \
        auto HYDRA_THRUST_PP_CAT2(count2, _fixed) = static_cast<hydra_thrust::detail::int64_t>(count2); \
        status = call arguments; \
    }
/**
 * Dispatch between 32-bit and 64-bit index based versions of the same algorithm
 ...
 */
#define HYDRA_THRUST_INDEX_TYPE_DISPATCH2(status, call_32, call_64, count, arguments) \
    if (count <= hydra_thrust::detail::integer_traits<hydra_thrust::detail::int32_t>::const_max) { \
        auto HYDRA_THRUST_PP_CAT2(count, _fixed) = static_cast<hydra_thrust::detail::int32_t>(count); \
