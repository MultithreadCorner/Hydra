 ...
 *  of interest from a range using the \p hydra_thrust::host execution policy for parallelization:
 *
 *  \code
 ...
 *  const int N = 6;
 *  int A[N] = {3, 1, 4, 1, 5, 9};
 *  int *new_end = hydra_thrust::remove(A, A + N, 1);
 ...
 *  a sequence of numbers to an output range while omitting a value of interest using the \p hydra_thrust::host
 ...
 *  int V[N] = {-2, 0, -1, 0, 1, 2};
 *  int result[N-2];
 *  hydra_thrust::remove_copy(hydra_thrust::host, V, V + N, result, 0);
 ...
 *  int V[N] = {-2, 0, -1, 0, 1, 2};
 *  int result[N-2];
 *  hydra_thrust::remove_copy(V, V + N, result, 0);
 *  // V remains {-2, 0, -1, 0, 1, 2}
 *  // result is now {-2, -1, 1, 2}
 *  \endcode
 *
 ...
 *  all even numbers from an array of integers using the \p hydra_thrust::host execution policy for
 *  parallelization:
 *
 *  \code
 *  #include <hydra/detail/external/hydra_thrust/remove.h>
 ...
 *  const int N = 6;
 *  int A[N] = {1, 4, 2, 8, 5, 7};
 *  int *new_end = hydra_thrust::remove_if(hydra_thrust::host, A, A + N, is_even());
 ...
 *  const int N = 6;
 *  int A[N] = {1, 4, 2, 8, 5, 7};
 *  int *new_end = hydra_thrust::remove_if(A, A + N, is_even());
 *  // The first three values of A are now {1, 5, 7}
 *  // Values beyond new_end are unspecified
 *  \endcode
 *
 ...
 *  a sequence of numbers to an output range while omitting even numbers using the \p hydra_thrust::host
 *  execution policy for parallelization:
 *
 *  \code
 *  #include <hydra/detail/external/hydra_thrust/remove.h>
 ...
 *  int V[N] = {-2, 0, -1, 0, 1, 2};
 *  int result[2];
 *  hydra_thrust::remove_copy_if(hydra_thrust::host, V, V + N, result, is_even());
 *  // V remains {-2, 0, -1, 0, 1, 2}
 *  // result is now {-1, 1}
 *  \endcode
 *
 ...
 *  int V[N] = {-2, 0, -1, 0, 1, 2};
 *  int result[2];
 *  hydra_thrust::remove_copy_if(V, V + N, result, is_even());
 *  // V remains {-2, 0, -1, 0, 1, 2}
 *  // result is now {-1, 1}
 *  \endcode
 *
 ...
 *
 *  The following code snippet demonstrates how to use \p remove_if to remove
 *  specific elements from an array of integers using the \p hydra_thrust::host execution policy for
 *  parallelization:
 *
 *  \code
 *  #include <hydra/detail/external/hydra_thrust/remove.h>
 ...
 *  int S[N] = {0, 1, 1, 1, 0, 0};
 *
 *  int *new_end = hydra_thrust::remove_if(hydra_thrust::host, A, A + N, S, hydra_thrust::identity<int>());
 *  // The first three values of A are now {1, 5, 7}
 *  // Values beyond new_end are unspecified
 *  \endcode
 *
 ...
 *  int S[N] = {0, 1, 1, 1, 0, 0};
 *
 *  int *new_end = hydra_thrust::remove_if(A, A + N, S, hydra_thrust::identity<int>());
 *  // The first three values of A are now {1, 5, 7}
 *  // Values beyond new_end are unspecified
 *  \endcode
 *
 ...
 *
 *  The following code snippet demonstrates how to use \p remove_copy_if to copy
 *  a sequence of numbers to an output range while omitting specific elements using the \p hydra_thrust::host
 *  execution policy for parallelization.
 *
 *  \code
 *  #include <hydra/detail/external/hydra_thrust/remove.h>
 ...
 *  int S[N] = { 1, 1,  0, 1, 0, 1};
 *  int result[2];
 *  hydra_thrust::remove_copy_if(hydra_thrust::host, V, V + N, S, result, hydra_thrust::identity<int>());
 *  // V remains {-2, 0, -1, 0, 1, 2}
 *  // result is now {-1, 1}
 *  \endcode
 *
 ...
 *  int S[N] = { 1, 1,  0, 1, 0, 1};
 *  int result[2];
 *  hydra_thrust::remove_copy_if(V, V + N, S, result, hydra_thrust::identity<int>());
 *  // V remains {-2, 0, -1, 0, 1, 2}
 *  // result is now {-1, 1}
 *  \endcode
 *
...
#include <hydra/detail/external/hydra_thrust/detail/remove.inl>
