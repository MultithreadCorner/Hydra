...
public:
	...
	inline result_type operator()(T&& rule_abscissa)
	{
        ...
        auto rule5_weight            = hydra_thrust::get<0>(rule_abscissa);
        auto rule7_weight            = hydra_thrust::get<1>(rule_abscissa);
		auto four_difference_weight  = hydra_thrust::get<2>(rule_abscissa);
		int four_diff_index          = hydra_thrust::get<3>(rule_abscissa);
	...
	//	std::cout<< four_diff_index<<std::endl;
		...
		_temp[0]      = fval*hydra_thrust::get<0>(rule_abscissa);//w5;
		_temp[1]      = fval*hydra_thrust::get<1>(rule_abscissa);//w7;
...
private:
	...
	*/
	...
	get_transformed_abscissa_helper(  Abscissa const& abscissa, TransAbscissa& transformed_abscissa  ){
		...
		hydra_thrust::get<I>(transformed_abscissa)  =
				fA[I]*hydra_thrust::get<I+4>( abscissa )+ fB[I];
...
//-----------------------
	...
	get_dim_helper( T const& X, int& result,  int& found_zeros ){
		...
		result +=  hydra_thrust::get<I+3>(X)>0.0 || hydra_thrust::get<I+3>(X) < 0.0 ?I:0.0;
		found_zeros  += int(!(hydra_thrust::get<I+3>(X)>0.0 || hydra_thrust::get<I+3>(X) < 0.0));
	...
	int get_dim( T const& X){
		...
		return found==int(N-1)?result: found==int(N)? int(N):-1 ;//< int(hydra_thrust::tuple_size<T>::value) ? result : -1;
	}
...
struct ProcessGenzMalikBox
{
	...
	inline void operator()(GenzMalikBox<N>& hyperbox) const
	{
		...
		typedef typename hydra_thrust::iterator_system<RuleIterator>::type System;
		using hydra_thrust::transform_reduce;
