 ...
 *  compact a sequence of numbers to remove consecutive duplicates using the \p hydra_thrust::host execution policy
 *  for parallelization:
 *
 *  \code
 ...
 *  int *new_end = hydra_thrust::unique(hydra_thrust::host, A, A + N);
 ...
 *  const int N = 7;
 *  int A[N] = {1, 3, 3, 3, 2, 2, 1};
 *  int *new_end = hydra_thrust::unique(A, A + N);
 *  // The first four values of A are now {1, 3, 2, 1}
 *  // Values beyond new_end are unspecified.
 *  \endcode
 *
 ...
 *  const int N = 7;
 *  int A[N] = {1, 3, 3, 3, 2, 2, 1};
 *  int *new_end = hydra_thrust::unique(hydra_thrust::host, A, A + N, hydra_thrust::equal_to<int>());
 *  // The first four values of A are now {1, 3, 2, 1}
 *  // Values beyond new_end are unspecified.
 *  \endcode
 *
 ...
 *  const int N = 7;
 *  int A[N] = {1, 3, 3, 3, 2, 2, 1};
 *  int *new_end = hydra_thrust::unique(A, A + N, hydra_thrust::equal_to<int>());
 *  // The first four values of A are now {1, 3, 2, 1}
 *  // Values beyond new_end are unspecified.
 *  \endcode
 *
 ...
 *  compact a sequence of numbers to remove consecutive duplicates using the \p hydra_thrust::host execution
 ...
 *  int A[N] = {1, 3, 3, 3, 2, 2, 1};
 *  int B[N];
 *  int *result_end = hydra_thrust::unique_copy(hydra_thrust::host, A, A + N, B);
 ...
 *  int A[N] = {1, 3, 3, 3, 2, 2, 1};
 *  int B[N];
 *  int *result_end = hydra_thrust::unique_copy(A, A + N, B);
 *  // The first four values of B are now {1, 3, 2, 1} and (result_end - B) is 4
 *  // Values beyond result_end are unspecified
 *  \endcode
 *
 ...
 *  int A[N] = {1, 3, 3, 3, 2, 2, 1};
 *  int B[N];
 *  int *result_end = hydra_thrust::unique_copy(hydra_thrust::host, A, A + N, B, hydra_thrust::equal_to<int>());
 *  // The first four values of B are now {1, 3, 2, 1} and (result_end - B) is 4
 *  // Values beyond result_end are unspecified.
 *  \endcode
 *
 ...
 *  int A[N] = {1, 3, 3, 3, 2, 2, 1};
 *  int B[N];
 *  int *result_end = hydra_thrust::unique_copy(A, A + N, B, hydra_thrust::equal_to<int>());
 *  // The first four values of B are now {1, 3, 2, 1} and (result_end - B) is 4
 *  // Values beyond result_end are unspecified.
 *  \endcode
 *
 ...
 *  compact a sequence of key/value pairs to remove consecutive duplicates using the \p hydra_thrust::host
 ...
 *
 *  hydra_thrust::pair<int*,int*> new_end;
 *  new_end = hydra_thrust::unique_by_key(hydra_thrust::host, A, A + N, B);
 *
 ...
 *
 *  hydra_thrust::pair<int*,int*> new_end;
 *  new_end = hydra_thrust::unique_by_key(A, A + N, B);
 *
 *  // The first four keys in A are now {1, 3, 2, 1} and new_end.first - A is 4.
 *  // The first four values in B are now {9, 8, 5, 3} and new_end.second - B is 4.
 *  \endcode
 ...
 *  new_end = hydra_thrust::unique_by_key(hydra_thrust::host, keys, keys + N, values, binary_pred);
 *
 *  // The first four keys in A are now {1, 3, 2, 1} and new_end.first - A is 4.
 *  // The first four values in B are now {9, 8, 5, 3} and new_end.second - B is 4.
 *  \endcode
 ...
 *  hydra_thrust::pair<int*,int*> new_end;
 *  hydra_thrust::equal_to<int> binary_pred;
 *  new_end = hydra_thrust::unique_by_key(keys, keys + N, values, binary_pred);
 *
 *  // The first four keys in A are now {1, 3, 2, 1} and new_end.first - A is 4.
 *  // The first four values in B are now {9, 8, 5, 3} and new_end.second - B is 4.
 *  \endcode
 ...
 *  compact a sequence of key/value pairs and with equal keys using the \p hydra_thrust::host execution policy
 *  for parallelization:
 *
 *  \code
 *  #include <hydra/detail/external/hydra_thrust/unique.h>
 ...
 *
 *  hydra_thrust::pair<int*,int*> new_end;
 *  new_end = hydra_thrust::unique_by_key_copy(hydra_thrust::host, A, A + N, B, C, D);
 *
 ...
 *
 *  hydra_thrust::pair<int*,int*> new_end;
 *  new_end = hydra_thrust::unique_by_key_copy(A, A + N, B, C, D);
 *
 *  // The first four keys in C are now {1, 3, 2, 1} and new_end.first - C is 4.
 *  // The first four values in D are now {9, 8, 5, 3} and new_end.second - D is 4.
 *  \endcode
 ...
 *
 *  The following code snippet demonstrates how to use \p unique_by_key_copy to
 *  compact a sequence of key/value pairs and with equal keys using the \p hydra_thrust::host execution policy for
 *  parallelization:
 *
 *  \code
 *  #include <hydra/detail/external/hydra_thrust/unique.h>
 ...
 *  hydra_thrust::pair<int*,int*> new_end;
 *  hydra_thrust::equal_to<int> binary_pred;
 *  new_end = hydra_thrust::unique_by_key_copy(hydra_thrust::host, A, A + N, B, C, D, binary_pred);
 *
 *  // The first four keys in C are now {1, 3, 2, 1} and new_end.first - C is 4.
 *  // The first four values in D are now {9, 8, 5, 3} and new_end.second - D is 4.
 *  \endcode
 ...
 *  hydra_thrust::pair<int*,int*> new_end;
 *  hydra_thrust::equal_to<int> binary_pred;
 *  new_end = hydra_thrust::unique_by_key_copy(A, A + N, B, C, D, binary_pred);
 *
 *  // The first four keys in C are now {1, 3, 2, 1} and new_end.first - C is 4.
 *  // The first four values in D are now {9, 8, 5, 3} and new_end.second - D is 4.
 *  \endcode
 ...
 *  determine a number of runs of equal elements using the \p hydra_thrust::host execution policy
 *  for parallelization:
 *
 *  \code
 *  #include <hydra/detail/external/hydra_thrust/unique.h>
 ...
 *  const int N = 7;
 *  int A[N] = {1, 3, 3, 3, 2, 2, 1};
 *  int count = hydra_thrust::unique_count(hydra_thrust::host, A, A + N, hydra_thrust::equal_to<int>());
 *  // count is now 4
 *  \endcode
 *
 *  \see unique_copy
 ...
 *
 *  The following code snippet demonstrates how to use \p unique_count to
 *  determine the number of runs of equal elements using the \p hydra_thrust::host execution policy
 *  for parallelization:
 *
 *  \code
 *  #include <hydra/detail/external/hydra_thrust/unique.h>
 ...
 *  const int N = 7;
 *  int A[N] = {1, 3, 3, 3, 2, 2, 1};
 *  int count = hydra_thrust::unique_count(hydra_thrust::host, A, A + N);
 *  // count is now 4
 *  \endcode
 *
 *  \see unique_copy
 ...
 *  const int N = 7;
 *  int A[N] = {1, 3, 3, 3, 2, 2, 1};
 *  int count = hydra_thrust::unique_count(A, A + N, hydra_thrust::equal_to<int>());
 *  // count is now 4
 *  \endcode
 *
 *  \see unique_copy
...
#include <hydra/detail/external/hydra_thrust/detail/unique.inl>
