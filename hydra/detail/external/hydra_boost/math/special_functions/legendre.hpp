...
T legendre_imp(unsigned l, T x, const Policy& pol, bool second = false)
{
   ...
   {
      std::swap(p0, p1);
      p1 = static_cast<T>(hydra_boost::math::legendre_next(n, x, p0, p1));
      ++n;
   }
   return p1;
}
...
)
{
   ...
   if (l == 0)
    ...
    {
        std::swap(p0, p1);
        *Pn = static_cast<T>(hydra_boost::math::legendre_next(n, x, p0, p1));
    }
    return p_prime;
}
...
T legendre_p_imp(int l, int m, T x, T sin_theta_power, const Policy& pol)
{
   ...
   if (-m == l)
   {
      return pow((1 - x * x) / 4, T(l) / 2) / hydra_boost::math::tgamma<T>(l + 1, pol);
   }
   if(m < 0)
   {
      int sign = (m&1) ? -1 : 1;
