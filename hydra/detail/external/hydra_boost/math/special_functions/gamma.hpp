...
#include <hydra/detail/external/hydra_boost/math/special_functions/detail/igamma_large.hpp>
#include <hydra/detail/external/hydra_boost/math/special_functions/detail/unchecked_factorial.hpp>
#include <hydra/detail/external/hydra_boost/math/special_functions/detail/lgamma_small.hpp>
...
T gamma_imp(T z, const Policy& pol, const Lanczos& l)
{
   ...
   else
   {
      ...
      {
         HYDRA_BOOST_MATH_INSTRUMENT_VARIABLE(zgh);
         HYDRA_BOOST_MATH_INSTRUMENT_VARIABLE(pow(zgh, T(z - hydra_boost::math::constants::half<T>())));
         HYDRA_BOOST_MATH_INSTRUMENT_VARIABLE(exp(zgh));
         result *= pow(zgh, T(z - hydra_boost::math::constants::half<T>())) / exp(zgh);
         HYDRA_BOOST_MATH_INSTRUMENT_VARIABLE(result);
      }
   }
   return result;
...
T lgamma_imp(T z, const Policy& pol, const Lanczos& l, int* sign = nullptr)
{
   ...
   {
      // regular evaluation:
      T zgh = static_cast<T>(z + T(Lanczos::g()) - hydra_boost::math::constants::half<T>());
      result = log(zgh) - 1;
      result *= z - 0.5f;
      //
...
T tgamma_delta_ratio_imp_lanczos(T z, T delta, const Policy& pol, const lanczos::undefined_lanczos& l)
{
   ...
   if (numerator_shift == 0 && denominator_shift == 0)
   {
      ...
      result *= exp(z * hydra_boost::math::log1p(-delta / (z + delta), pol)) * pow(T((delta + z) / constants::e<T>()), -delta);
      return result;
   }
   //
...
#include <hydra/detail/external/hydra_boost/math/special_functions/detail/igamma_inverse.hpp>
