...
#ifndef HYDRA_BOOST_MATH_CCMATH_FMA_HPP
#define HYDRA_BOOST_MATH_CCMATH_FMA_HPP
...
constexpr Real fma(Real x, Real y, Real z) noexcept
{
    ...
    {
        if (x == 0 && hydra_boost::math::ccmath::isinf(y))
        {
            return std::numeric_limits<Real>::quiet_NaN();
        }
        else if (y == 0 && hydra_boost::math::ccmath::isinf(x))
        {
            return std::numeric_limits<Real>::quiet_NaN();
        }
        ...
        else if (hydra_boost::math::ccmath::isnan(z))
        {
            return std::numeric_limits<Real>::quiet_NaN();
        }

        return hydra_boost::math::ccmath::detail::fma_imp(x, y, z);
    }
    else
    {
        using std::fma;
...
constexpr auto fma(T1 x, T2 y, T3 z) noexcept
{
    if (HYDRA_BOOST_MATH_IS_CONSTANT_EVALUATED(x))
    {
        ...
        return hydra_boost::math::ccmath::fma(promoted_type(x), promoted_type(y), promoted_type(z));
    }
    else
    {
        using std::fma;
...
{
    return hydra_boost::math::ccmath::fma(x, y, z);
}
...
#endif // HYDRA_BOOST_MATH_CCMATH_FMA_HPP
