...
#ifndef HYDRA_BOOST_MATH_STATISTICS_CHATTERJEE_CORRELATION_HPP
#define HYDRA_BOOST_MATH_STATISTICS_CHATTERJEE_CORRELATION_HPP
...
#include <hydra/detail/external/hydra_boost/math/statistics/detail/rank.hpp>
...
ReturnType chatterjee_correlation_seq_impl(ForwardIterator u_begin, ForwardIterator u_end, ForwardIterator v_begin, ForwardIterator v_end)
{
    ...
    HYDRA_BOOST_MATH_ASSERT_MSG(std::is_sorted(u_begin, u_end), "The x values must be sorted in order to use this functionality");
...
{
    using std::abs;
    HYDRA_BOOST_MATH_ASSERT_MSG(std::is_sorted(std::forward<ExecutionPolicy>(exec), u_begin, u_end), "The x values must be sorted in order to use this functionality");
...
#endif // HYDRA_BOOST_MATH_STATISTICS_CHATTERJEE_CORRELATION_HPP
