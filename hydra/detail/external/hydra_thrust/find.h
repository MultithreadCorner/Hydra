 ...
 *  ...
 *  hydra_thrust::device_vector<int> input(4);
 *
 *  input[0] = 0;
 *  input[1] = 5;
 *  input[2] = 3;
 *  input[3] = 7;
 *
 *  hydra_thrust::device_vector<int>::iterator iter;
 *
 *  iter = hydra_thrust::find(hydra_thrust::device, input.begin(), input.end(), 3); // returns input.first() + 2
 *  iter = hydra_thrust::find(hydra_thrust::device, input.begin(), input.end(), 5); // returns input.first() + 1
 *  iter = hydra_thrust::find(hydra_thrust::device, input.begin(), input.end(), 9); // returns input.end()
 *  \endcode
 *
 *  \see find_if
 *  \see mismatch
 ...
 *  hydra_thrust::device_vector<int>::iterator iter;
 *
 *  iter = hydra_thrust::find(input.begin(), input.end(), 3); // returns input.first() + 2
 *  iter = hydra_thrust::find(input.begin(), input.end(), 5); // returns input.first() + 1
 *  iter = hydra_thrust::find(input.begin(), input.end(), 9); // returns input.end()
 *  \endcode
 *
 *  \see find_if
 *  \see mismatch
 ...
 *  hydra_thrust::device_vector<int>::iterator iter;
 *
 *  iter = hydra_thrust::find_if(hydra_thrust::device, input.begin(), input.end(), greater_than_four()); // returns input.first() + 1
 *
 *  iter = hydra_thrust::find_if(hydra_thrust::device, input.begin(), input.end(), greater_than_ten());  // returns input.end()
 *  \endcode
 *
 *  \see find
 *  \see find_if_not
 ...
 *  hydra_thrust::device_vector<int>::iterator iter;
 *
 *  iter = hydra_thrust::find_if(input.begin(), input.end(), greater_than_four()); // returns input.first() + 1
 *
 *  iter = hydra_thrust::find_if(input.begin(), input.end(), greater_than_ten());  // returns input.end()
 *  \endcode
 *
 *  \see find
 *  \see find_if_not
 ...
 *  hydra_thrust::device_vector<int>::iterator iter;
 *
 *  iter = hydra_thrust::find_if_not(hydra_thrust::device, input.begin(), input.end(), greater_than_four()); // returns input.first()
 *
 *  iter = hydra_thrust::find_if_not(hydra_thrust::device, input.begin(), input.end(), greater_than_ten());  // returns input.first()
 *  \endcode
 *
 *  \see find
 *  \see find_if
 ...
 *  hydra_thrust::device_vector<int>::iterator iter;
 *
 *  iter = hydra_thrust::find_if_not(input.begin(), input.end(), greater_than_four()); // returns input.first()
 *
 *  iter = hydra_thrust::find_if_not(input.begin(), input.end(), greater_than_ten());  // returns input.first()
 *  \endcode
 *
 *  \see find
 *  \see find_if
...
#include <hydra/detail/external/hydra_thrust/detail/find.inl>
