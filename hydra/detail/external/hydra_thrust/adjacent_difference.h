 ...
 *  the difference between adjacent elements of a range using the \p hydra_thrust::device execution policy:
 *
 *  \code
 ...
 *  hydra_thrust::device_vector<int> d_data(h_data, h_data + 8);
 *  hydra_thrust::device_vector<int> d_result(8);
 *
 *  hydra_thrust::adjacent_difference(hydra_thrust::device, d_data.begin(), d_data.end(), d_result.begin());
 *
 ...
 *
 *  The following code snippet demonstrates how to use \p adjacent_difference to compute
 *  the sum between adjacent elements of a range using the \p hydra_thrust::device execution policy:
 *
 *  \code
 *  #include <hydra/detail/external/hydra_thrust/adjacent_difference.h>
 ...
 *  hydra_thrust::device_vector<int> d_result(8);
 *
 *  hydra_thrust::adjacent_difference(hydra_thrust::device, d_data.begin(), d_data.end(), d_result.begin(), hydra_thrust::plus<int>());
 *
 ...
 *  hydra_thrust::device_vector<int> d_result(8);
 *
 *  hydra_thrust::adjacent_difference(d_data.begin(), d_data.end(), d_result.begin());
 *
 *  // d_result is now [1, 1, -1, 1, -1, 1, -1, 1]
 *  \endcode
 *
 ...
 *  hydra_thrust::device_vector<int> d_result(8);
 *
 *  hydra_thrust::adjacent_difference(d_data.begin(), d_data.end(), d_result.begin(), hydra_thrust::plus<int>());
 *
 *  // d_result is now [1, 3, 3, 3, 3, 3, 3, 3]
 *  \endcode
 *
