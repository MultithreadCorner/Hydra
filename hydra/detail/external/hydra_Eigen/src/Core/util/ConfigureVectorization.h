...
#ifndef HYDRA_EIGEN_CONFIGURE_VECTORIZATION_H
#define HYDRA_EIGEN_CONFIGURE_VECTORIZATION_H
 ...
 */
  ...
  #define HYDRA_EIGEN_ALIGNOF(x) __alignof(x)
#elif HYDRA_EIGEN_HAS_ALIGNAS
  #define HYDRA_EIGEN_ALIGN_TO_BOUNDARY(n) alignas(n)
  #define HYDRA_EIGEN_ALIGNOF(x) alignof(x)
...
// HYDRA_EIGEN_DONT_ALIGN_STATICALLY and HYDRA_EIGEN_DONT_ALIGN are deprecated
// They imply HYDRA_EIGEN_MAX_STATIC_ALIGN_BYTES=0
  ...
  #if HYDRA_EIGEN_COMP_GNUC && !(HYDRA_EIGEN_ARCH_i386_OR_x86_64 || HYDRA_EIGEN_ARCH_ARM_OR_ARM64 || HYDRA_EIGEN_ARCH_PPC || HYDRA_EIGEN_ARCH_IA64 || HYDRA_EIGEN_ARCH_MIPS)
...
// If HYDRA_EIGEN_MAX_ALIGN_BYTES is defined, then it is considered as an upper bound for HYDRA_EIGEN_MAX_STATIC_ALIGN_BYTES
...
// It takes into account both the user choice to explicitly enable/disable alignment (by setting HYDRA_EIGEN_MAX_STATIC_ALIGN_BYTES)
...
// removed as gcc 4.1 and msvc 2008 are not supported anyways.
  ...
  #include <malloc.h> // for _aligned_malloc -- need it regardless of whether vectorization is enabled
...
#if !(defined(HYDRA_EIGEN_DONT_VECTORIZE) || defined(HYDRA_EIGEN_GPUCC))
    ...
    // want to force the use of those instructions with msvc.
      ...
      #define HYDRA_EIGEN_VECTORIZE_SSE4_2
    #endif
    #if defined(__FMA__) || (HYDRA_EIGEN_COMP_MSVC && defined(__AVX2__))
      // MSVC does not expose a switch dedicated for FMA
      // For MSVC, AVX2 => FMA
      #define HYDRA_EIGEN_VECTORIZE_FMA
    #endif
    #if defined(__AVX512F__)
      #ifndef HYDRA_EIGEN_VECTORIZE_FMA
      ...
      #define HYDRA_EIGEN_VECTORIZE_SSE4_2
        ...
        #endif
        #ifdef __AVX512ER__
          #define HYDRA_EIGEN_VECTORIZE_AVX512ER
        #endif
        #ifdef __AVX512BF16__
          #define HYDRA_EIGEN_VECTORIZE_AVX512BF16
        #endif
      #endif
    #endif
    ...
    // Disable AVX support on broken xcode versions
      ...
      // when XCode 11.0 and Mac deployment target macOS 10.15 is https://trac.macports.org/ticket/58776#no1
        ...
        #undef HYDRA_EIGEN_VECTORIZE_AVX
        #warning "Disabling AVX support: clang compiler shipped with XCode 11.[012] generates broken assembly with -macosx-version-min=10.15 and AVX enabled. "
          ...
          #undef HYDRA_EIGEN_VECTORIZE_AVX2
        #endif
        #ifdef HYDRA_EIGEN_VECTORIZE_FMA
          #undef HYDRA_EIGEN_VECTORIZE_FMA
        #endif
        #ifdef HYDRA_EIGEN_VECTORIZE_AVX512
          #undef HYDRA_EIGEN_VECTORIZE_AVX512
        #endif
        #ifdef HYDRA_EIGEN_VECTORIZE_AVX512DQ
          #undef HYDRA_EIGEN_VECTORIZE_AVX512DQ
        #endif
        #ifdef HYDRA_EIGEN_VECTORIZE_AVX512ER
          #undef HYDRA_EIGEN_VECTORIZE_AVX512ER
        #endif
      #endif
  ...
  #elif ((defined  __ARM_NEON) || (defined __ARM_NEON__)) && !(defined HYDRA_EIGEN_ARM64_USE_SVE)

    #define HYDRA_EIGEN_VECTORIZE
    #define HYDRA_EIGEN_VECTORIZE_NEON
    #include <arm_neon.h>

  // We currently require SVE to be enabled explicitly via HYDRA_EIGEN_ARM64_USE_SVE and
  // will not select the backend automatically
  #elif (defined __ARM_FEATURE_SVE) && (defined HYDRA_EIGEN_ARM64_USE_SVE)

    #define HYDRA_EIGEN_VECTORIZE
    #define HYDRA_EIGEN_VECTORIZE_SVE
    #include <arm_sve.h>

    // Since we depend on knowing SVE vector lengths at compile-time, we need
    // to ensure a fixed lengths is set
    #if defined __ARM_FEATURE_SVE_BITS
      #define HYDRA_EIGEN_ARM64_SVE_VL __ARM_FEATURE_SVE_BITS
    #else
#error "Eigen requires a fixed SVE lector length but HYDRA_EIGEN_ARM64_SVE_VL is not set."
#endif
...
#if defined(__LP64__)
#define HYDRA_EIGEN_MIPS_64
#else
#define HYDRA_EIGEN_MIPS_32
#endif
#define HYDRA_EIGEN_VECTORIZE
#define HYDRA_EIGEN_VECTORIZE_MSA
#include <msa.h>
#endif
...
// See also: https://bugs.llvm.org/show_bug.cgi?id=47955
...
#if defined(__F16C__) && (!defined(HYDRA_EIGEN_GPUCC) && (!defined(HYDRA_EIGEN_COMP_CLANG) || HYDRA_EIGEN_COMP_CLANG>=380))
  // We can use the optimized fp16 to float and float to fp16 conversion routines
  #define HYDRA_EIGEN_HAS_FP16_C

  #if defined(HYDRA_EIGEN_COMP_CLANG)
...
#if defined HYDRA_EIGEN_CUDACC
  #define HYDRA_EIGEN_VECTORIZE_GPU
  #include <vector_types.h>
  #if HYDRA_EIGEN_CUDA_SDK_VER >= 70500
  ...
  #define HYDRA_EIGEN_HAS_HIP_FP16
...
#endif // HYDRA_EIGEN_CONFIGURE_VECTORIZATION_H
